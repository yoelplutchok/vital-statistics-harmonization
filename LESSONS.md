...
## 2026-05-22T02:00:00Z — C8.18_step6a-RECWT — L13-extension (year-axis) addendum — the 2026-05-20 (A′) "RECWT@91 era-unstable" conclusion was FALSIFIED at the §2 cheap-check: a single ≤5000-row head-sample byte-equality probe is NOT a per-year byte-stability proof

> This entry is the owed §11 LESSONS addendum that DECISION_LOG 2026-05-22T02:00:00Z explicitly flagged ("the LESSONS 2026-05-20 (A′) entry is partially superseded — a §11 LESSONS addendum is owed: a single 5000-row head-sample byte-equality probe is NOT a per-year byte-stability proof; verify against the file-level NCHS control total"). It partially supersedes the **(A′)** finding of the 2026-05-20T02:00:00Z entry below (append-only-supersede §3; that entry is preserved). The **(A″)** `from_pylist` heterogeneous-union finding of the 2026-05-20 entry is UNCHANGED and stands.

**What happened:** The 2026-05-20 (A′) finding concluded `field_specs.LINKED_BIRTH_1983_1988_FIELDS`'s `RECWT@91` is "era-unstable for non-anchor purposes" because a read-only 5000-row head-sample probe found `byte91 == DMRACE@57` for **100%** of the 1988 head rows (and the layout was 3a-derived from the single 1983 guide, anchors-only-verified). The conservative response was `record_weight = NULL` for all of 1983-1988. At C8.18 DO step 6a the L9 guide control totals revealed the 1983-1984 cohort **denominator** is a documented 50%-of-births-in-5-non-VSCP-areas weighted sample (den Record count ≠ by-occurrence), so NULL-weight made the 1983-1984 IMR ~9% biased — a binding §7 halt-and-ask. The §2 cheap-check that resolved it (read-only `LinkCO83Guide.pdf` p4/p12/p13 coordinate extraction + a full-`_raw` Σ probe over the entire parsed 1983 denominator, 3,341,274 rows) **FALSIFIED the (A′) "era-unstable" conclusion**: Σ(RECWT@91) over the full 1983 den = 3,643,001 == the guide "By occurrence" total **byte-exact**, with values ∈ {"1","2"} exactly per the p4 methodology. The 100% head-sample coincidence was an **artifact of file ordering** — the denominator file is ordered by State-of-occurrence (p4), so the first 5000 records are all VSCP weight-"1" and `DMRACE@57`≈"1"=White; the `byte91 == byte57` equality is only ~75-81% on the FULL 1983-1987 file, not 100%. (1988 IS a genuine trailing-byte anomaly — 100%, multi-valued — but 1988 is a full file [Record count == by-occurrence] needing NO weighting, so `record_weight = NULL` stays faithful there.) `RECWT@91` is therefore **CORRECT** for the cohort denominator; the 1983-1984 fix was harmonize-only (populate `record_weight` from RECWT@91, era-gated), no `field_specs.py` position edit, no re-parse.

**Why the existing matrix didn't catch it:** It is still **L13-extension (year-axis)** — the right class — but the 2026-05-20 entry applied the *probe* too weakly: a small head-sample byte-equality is not a per-year byte-stability proof. The matrix sharpening must name the **file-level NCHS control total** (the source's own published aggregate for the parsed field) as the verification target, not a head-sample.

**What worked:** §2 cheap-before-expensive — the read-only guide-PDF coordinate extraction + the full-`_raw` Σ-vs-NCHS-file-level-control-total probe (NOT a head-sample) FALSIFIED the assumed defect before any `field_specs.py`/re-parse. The established within-task "prior assumption FALSIFIED → smaller correct scope" precedent (5a/4a/4b) routed the response.

**Proposed matrix-row sharpening (§11 — human-merge):** APPLIED in this same `[plan-update]` as the §8 row **`L13-ext (year-axis)`** (its "Caught at" column carries the explicit caveat: a single ≤5000-row head-sample byte-equality is itself NOT a per-year proof; value-distribution-verify each year's non-anchor fields **against the source's own file-level NCHS control total**).

**Backport scope:** Within C8.18 — the 5c-iii `record_weight`=NULL decision is superseded for **1983-1984** by 6a-RECWT (correct, populated from RECWT@91) and remains correct for **1985-1988** (full files; unweighted == published-comparable; the genuine 1988 trailing-byte anomaly needs no weighting). The 2026-05-20 entry's (A″) `from_pylist` root-cause fix is unaffected. No cross-product backport — no other HVS product uses a single-year-derived multi-year cohort layout carrying a sampling weight (natality/fetal-death/MM/linked-2005+ are single-member homogeneous).

**Upstream lesson?** Yes — generalizes: any single-year-derived multi-year layout's non-anchor byte-stability claim drawn from a head-sample is unproven; verify the parsed field's aggregate against the source's own file-level control total before concluding "era-unstable" (the conservative-NULL is still correct as an interim, but the "unstable" *conclusion* must clear the control-total bar).

---

## 2026-05-20T02:00:00Z — C8.18_step5c-iii — L13-extension (year-axis) + H6 (heterogeneous-union materialization) — A single-source-derived layout value-verified only on anchor fields + one era-year hides a non-anchor byte re-purposing at another era-year of the same record length; AND a lossless multi-segment union materialized via first-record-schema-inference silently drops the late-arriving segment's columns

**What happened (two related findings, both surfaced at the 5c-iii Convention-3/SMOKE cheap-check — the system working as designed; the 3a self-check #2 + 5b self-check #3 had explicitly scheduled this per-year verification for "DO step 5c/6"):**

**(A′) RECWT@91 year-axis non-anchor instability.** `field_specs.LINKED_BIRTH_1983_1988_FIELDS` was 3a-authored by transcribing the **1983** cohort-linked guide, and only anchor fields (BIRYR/MATCHS/CSEX/DPLURAL/DBIRWT) were value-verified on real 1983/85/88 data. The 5c-iii read-only Convention-3 raw-byte probe found `RECWT@91` (the trailing byte of the 91-byte denominator record) is uniformly `"1"` for 1983 and 1985 (a clean 100%-sample inflation weight) but is **byte-for-byte identical to `DMRACE@57`** in 1988 (5000/5000 records). The single-layout-for-all-of-1983-1988 assumption is byte-stable for the record LENGTH (91) and for bytes 1-90, but NOT for the trailing byte 91 across 1986-1988. A "record weight" cannot equal detail-race-of-mother — the layout's `RECWT@91` is era-unstable for non-anchor purposes.

**(A″) from_pylist-first-record heterogeneous-union silent-column-drop.** `pa.Table.from_pylist` / `pa.RecordBatch.from_pylist` infer the Arrow schema from the **first record's keys ONLY** (empirically confirmed: 20 den-then-20-num dicts → the entire numerator section dropped). `parse_linked_year.run_parse` (RESOLVED-5b code) materializes the parser stream via `pa.Table.from_pylist(rows/buffer)`, and the RESOLVED-5b `_iter_two_file_1983_1988` yields **ALL den rows (47 keys, no death section) BEFORE any num row (74 keys, with the ICD-9 mortality section)**. So at DO step 6 the 1983-1988 `_raw` parquet build would **silently drop the entire numerator ICD-9/age-recode section** (single-list path) or **crash** the chunked write at the den→num boundary — defeating the 5b receipt's "Lossless; H6 row-count conservation" claim (which was true for ROW count but never tested for the heterogeneous COLUMN union via `run_parse`'s `from_pylist`; the 5b SMOKE read members directly, ≤400 records, never via `run_parse`).

**Why the existing matrix didn't catch it:**
- (A′) is **L13-extension** (already in LESSONS 2026-05-12T01:40:00Z as the MAGER-vs-MAGER41 byte-position-stable-but-semantically-re-purposed case). The new wrinkle is the **YEAR axis**: the prior L13-extension was sibling-layout-vs-sibling-layout at one year; this is the SAME layout label across DIFFERENT era-years of the same record length, where a non-anchor byte re-purposes mid-era. The matrix's L13-extension "compute the parsed value distribution and verify it matches the user guide" catches it IF the verification is done per-era-year, not just on anchors + one year.
- (A″) is closest to **H6** ("Silent row drops in joins" — extended to silent COLUMN drops in a heterogeneous-union materialization) and **L13-extension** (a declarative materialization assumption — "from_pylist round-trips the dicts losslessly" — being incomplete for a heterogeneous-key stream). The matrix did not explicitly name the "schema inferred from the first record only" failure of `from_pylist` on a den-then-num-ordered heterogeneous union.

**What worked:** §9-#9 (SMOKE authored FIRST + run on real `LinkCO83`/`LinkCO88` data) surfaced (A″) at the cheap-check moment (the Tier-1 `assert icd9` failed because the numerator UCOD was dropped). The read-only Convention-3 raw-byte probe (`byte57==byte91` per-year cross-check on real den members) surfaced (A′) before any code mutation. §2 cheap-before-expensive + the binding §7/§8 halt-and-ask (AskUserQuestion for the (A″) RESOLVED-5b defect) routed the response correctly: (A′) resolved within 5c-iii scope via the faithful no-patch conservative-NULL (the 5c-i/1989-1991 precedent; `field_specs.py` UNTOUCHED); (A″) surfaced + human-authorized + fixed at the root cause (an explicit unified den∪num schema for 1983-1988 only, byte-identical for every homogeneous year).

**Proposed matrix-row sharpening (§11 — human-merge):**

> **L13-extension (year-axis sharpening)** — A layout reconstructed from a SINGLE source year/guide and value-verified only on anchor fields is NOT proven byte-stable for non-anchor fields at OTHER era-years of the same record length. Before harmonize-mapping a multi-year era from a single-year-derived layout, value-distribution-verify EACH year's non-anchor fields (esp. trailing/last-byte fields) — a per-year `byte_i == byte_j` cross-tab or per-year distribution catches a mid-era byte re-purposing. **Caught at:** PRE-FLIGHT Convention-3 read-only real-data probe (per-era-year, not anchors + one year). Conservative-NULL the non-byte-stable field; route the full per-year re-verification to the step that builds + NCHS-validates the parquet.
>
> **H6 (heterogeneous-union materialization sharpening)** — `pa.Table.from_pylist` / `pa.RecordBatch.from_pylist` infer the Arrow schema from the FIRST record's keys ONLY. A lossless multi-segment union materialized this way, where segments have DIFFERENT key sets and the narrower segment is yielded first, SILENTLY DROPS the late-arriving segment's columns (or crashes a chunked ParquetWriter at the segment boundary). **Caught at:** SMOKE on real data (§9-#9) — assert the late-segment-only columns survive the parse→parquet→read round-trip. **Specific catch:** any `from_pylist`/`from_pandas` materialization of a heterogeneous-key stream MUST pass an explicit unified schema (derived from the layout, not from data order); add a SMOKE that round-trips a first-segment-then-second-segment fixture and asserts the second segment's exclusive columns are non-null.

**Backport scope:** Within C8.18: (A′) resolved at 5c-iii (record_weight NULL); the FULL per-year (1984-1988) non-anchor byte-stability re-verification = soft-flag (ll) DO step 6 PRE-FLIGHT (the NCHS-per-year-cohort-count §15.D VERIFY is the durable gate; `field_specs.py` untouched — a per-year layout reconstruction, if the DO-6 verification shows other non-anchor drift, is a 3a-class task). (A″) root-cause-fixed at 5c-iii (`_expected_parsed_schema` + the `run_parse` wiring; 1983-1988 only; homogeneous years byte-identical). No other product uses a two-file heterogeneous den/num union (natality/fetal-death/MM/linked-2005+ are single-member homogeneous) → no cross-product backport; the 5b receipt's "Lossless" claim is corrected by this entry + the 5c-iii receipt (append-only-supersede). DO step 6's actual full-file re-harmonize + the NCHS-per-year-cohort-count VERIFY is the durable end-to-end gate that both findings are correctly resolved.

**Upstream lesson?** Yes — H6 (heterogeneous-union materialization) is general: any LLM-amplifier harmonization that materializes a multi-segment/heterogeneous-key record stream via a first-record-schema-inferring API is at risk; pass an explicit schema. The L13-extension year-axis sharpening generalizes to any multi-year era harmonized from a single-year-derived layout.
