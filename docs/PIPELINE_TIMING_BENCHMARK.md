...
>
> - **Natality + linked pipeline section + the H10 linked-SHA row.** Measured against natality **1990-2024 / 138.8M** (pre-C8.17) and **linked v3 2005-2023 / 74.9M / 94 cols / SHA `9b828a4de4e59b17…`**. The current shipped envelope is natality **v3.0.0 1968-2024 / 201,161,456 / 57 yr** (C8.17, 2026-05-14) and **linked v4.0.0 1983-2023 / 149,386,620 / 97 cols / SHA `f630d8cf…`** (C8.18, 2026-05-23; the `9b828a4d…` SHA is now the `.v3_baseline`). The natality+linked **wall-clock numbers + the §H10 linked row are therefore the pre-C8.17/pre-C8.18 measurement**, not the v4 figures.
...
>
> **Why not re-measured here:** a faithful v4 re-measure requires a multi-hour end-to-end re-run of the canonical natality (201M) + linked-v4 (149M) pipeline — a canonical-pipeline rebuild that is out of a pre-Zenodo documentation-cleanup's scope (C8.7b-orchestrator-DEFERRED class; §7-#17). Fabricating v4 wall-clock numbers without measuring would violate L6 (no invented numbers). The **manuscript timing-claim reconciliation is already routed to Phase D step 4** (the C8.13 fetal-death PROPOSE-EDIT, §below); the v4 natality+linked re-measure belongs to that same Phase-D post-final-rebuild pass. This note converts soft-flag (ee) from an undocumented latent staleness into a documented, scoped limitation; the methodology + driver scripts below reproduce a v4 measurement when a Phase-D rebuild is run.
