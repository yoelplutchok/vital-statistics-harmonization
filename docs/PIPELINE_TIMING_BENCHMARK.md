...
> **⚠ Post-C8.13 scope note (added 2026-05-23, `convenience-benchmark-v4-scope`; soft-flag (ee)).** This is a **point-in-time C8.13 measurement (2026-05-13)** against the envelope current *that day*. The data envelope has since grown, so two parts of this document are **superseded point-in-time figures** (deliberately NOT re-measured here — see why below):
>
> - **Natality + linked pipeline section + BOTH H10 SHA rows (natality and linked).** Measured against natality **1990-2024 / 138.8M** (pre-C8.17; the §H10 row `natality_v2_harmonized_derived.parquet` = `e16ad5323d68e28d…` is the pre-v3.0.0 anchor) and **linked v3 2005-2023 / 74.9M / 94 cols / SHA `9b828a4de4e59b17…`**. The current shipped envelope is natality **v3.0.0 1968-2024 / 201,161,456 / 57 yr / derived SHA `acb5c48a9abf82ac…`** (C8.17, 2026-05-14) and **linked v4.0.0 1983-2023 / 149,386,620 / 97 cols / SHA `f630d8cf…`** (C8.18, 2026-05-23; the `9b828a4d…` SHA is now the `.v3_baseline`). The natality+linked **wall-clock numbers + BOTH §H10 SHA rows (natality `e16ad532…` AND linked `9b828a4d…`) are therefore pre-C8.17/pre-C8.18 point-in-time figures**, not the current v4 envelope (`acb5c48a…` + `f630d8cf…`).
> - The **fetal-death pipeline section, Measurement methodology, Caveats, and How-to-reproduce remain valid** — fetal-death was already at v2.4.0 (1982-2024 / 2,427,233) when C8.13 measured it.
>
> **Why not re-measured here:** a faithful v4 re-measure requires a multi-hour end-to-end re-run of the canonical natality (201M) + linked-v4 (149M) pipeline — a canonical-pipeline rebuild that is out of a pre-Zenodo documentation-cleanup's scope (C8.7b-orchestrator-DEFERRED class; §7-#17). Fabricating v4 wall-clock numbers without measuring would violate L6 (no invented numbers). The C8.13 fetal-death PROPOSE-EDIT body (§below) routes the **fetal-death** manuscript-timing edit to **Phase D step 4**; the v4 **natality+linked** re-measure is a separate Phase-D post-final-rebuild item, routed to D.4 by this scope note (STATUS 2026-05-23 carried-forward), not by the C8.13 PROPOSE-EDIT body itself (whose recommended edit at L98 below is fetal-death-only — the natality clause at L146 is "no edit recommended"). This note converts soft-flag (ee) from an undocumented latent staleness into a documented, scoped limitation; the methodology + driver scripts below reproduce a v4 measurement when a Phase-D rebuild is run.
