...
- **C8.18** — **Linked birth–infant death 1983–2004 cohort backward extension → linked v4.0.0.** Linked coverage extended from 2005–2023 (19 years) to **1983–2023 (38 years; permanent 1992–1994 NCHS-linkage gap — NCHS suspended ALL birth-infant-death linkage for those three cohorts)**: 19 pre-2005 NCHS cohort linked-file zips parsed (1983–1991 keyless two-file den/num; 1995–2004 denominator-plus) and re-harmonized into the linked schema. Canonical parquet is now **149,386,620 records** (= 74,442,796 pre-2005 cohort + 74,943,824 v3 2005–2023, exactly). The 2005–2023 slice is byte-clean (verified vs the preserved `.v3_baseline` parquet). Three within-era cohort columns added — `underlying_cause_icd9` + `cause_recode_61` (ICD-9 cause-of-death for 1983–1998; no published 9→10 crosswalk applied — a deferred derivation task) and `link_segment` (the keyless 1983–1988 den/num tag): harmonized 78→81, derived 94→97; `harmonized_schema.csv` 94→97 rows, `years_available` widened to 1983–2023 with the 1992–1994 gap. Validation: existing 2005–2023 35/35 (33 byte-exact + 2 cells differ by exactly 1 record within documented tolerance) preserved byte-exact; pre-2005 cohort 1983–2004 validated per-year against the NCHS cohort linked-file user guides — cohort denominator + resident births byte-exact 19/19, published IMR within ±0.02 19/19, weighted 1983–1984 (a documented 50%-non-VSCP RECWT-weighted sample) byte-exact, and a documented numerator-file-vs-denominator-linkage residual for 1989/1998/2002 (3 of 19; the SAME NCHS cohort-linked class as the 2 differ-by-1 cells; pinned as a per-year tolerance with sourced notes). Keyless 1983–1988 within-era structural difference: IMR = `count(link_segment='num')/count(link_segment='den')` per stratum, NOT a per-birth `infant_death` filter (documented in schema/CODEBOOK/COMPARABILITY; the manuscript Coverage re-paragraph is a Phase-D / D.4 deliverable). Linked product version bump v3 → **v4.0.0** (Anti-Pattern #6: schema edit paired with the version bump + DECISION_LOG 2026-05-23T02:00:00Z); the canonical parquet **filename `natality_v3_linked_*` is retained** (the `v3` is the linked schema-family tag — the natality `.v28_baseline` filename-retention precedent). Schema-additive over the v3 2005–2023 surface — existing 2005–2023 analyses run unchanged. The 10 non-linked-derived canonical parquet SHAs (natality-v2 ×4 incl. `.v28_baseline`, fetal-death ×2, matched-multiples ×4) are byte-exact; only the linked-derived gate SHA changed (`9b828a4d…` → `f630d8cf…`, by design — the single C8.18 canonical mutation). (Receipts: `RECEIPTS/C8.18_step1…step7_*.md`.)
...
- **C8.2** — Latest-year fetal-death refresh: NCHS public-use 2023 + 2024 files harmonized into the existing v2.0 + V2.1 + V3a + V3b envelope. Fetal-death coverage now spans **1982–2024 (43 years, 2,427,233 records)**. NVSR per-year validation cells re-extended through 2024. (Receipt: `RECEIPTS/C8.2_2026-05-12T23-30-00Z.md`.)
