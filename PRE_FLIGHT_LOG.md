...
### Inputs
- [x] Read-only inputs only: the 4 settled-envelope parquets (gate SHAs already verified at 08:00:00Z — linked-v4 derived `f630d8cf…`, fetal-death derived `185c071e…`; natality v3.0.0 + matched-multiples context). The analysis script will do filtered+projected read-only `pyarrow.dataset` reads (no `/tmp` canonical writes; the existing `output/` parquets are inputs only).
- [x] Upstream complete: C8.18 COMPLETE; the §11 re-scope `[plan-update]` `6912534` is HEAD's parent's child (committed; `git log` confirms `6912534` after `1902415`).
- [x] WebFetch sources captured (L9): `https://www.cdc.gov/nchs/data_access/vitalstatsonline.htm` (no NCHS perinatal-record file/tabulation) + `https://www.cdc.gov/rdc/index.html` (RDC = restricted-use; public RDC page describes indirect identifiers / geographic detail but NOT a maternal cross-file/cross-pregnancy linkage key; contact rdca@cdc.gov) — to be cited in the future-work section, not over-claimed.

### Environment
- [x] Python 3.13.9 / pandas 2.3.2 / pyarrow 18.1.0 ✓; git clean; HEAD `6912534` on `main` (the `[plan-update]` commit) ✓.

### Outputs
- [x] Intended output paths do **not** exist (verified `ls`): `docs/PERINATAL_RECORD_FEASIBILITY.md` (NEW; `docs/` = cross-product docs — correct home), `scripts/perinatal_feasibility_analysis.py` (NEW; `scripts/` houses analysis/driver scripts — `_drive_*`/`_time_pipeline.py` siblings). Pointer-integration targets exist + will receive minimal additive edits: `PROJECT_STRUCTURE.md` (which-file-by-use-case matrix — add a "pre-joined perinatal record" row), `docs/JOINT_USE_GUIDE.md` (perinatal §128 — one-line pointer), `docs/WORKED_EXAMPLE_FAQ.md` (a FAQ entry). No manuscript edit (D.4 per KICKOFF).

### Field-value snapshot for cells / rows / columns being mutated (Convention 3)
- Zero canonical artifact mutated (no parquet / `harmonized_schema.csv` / validation-CSV / test / metadata-CSV). Purely additive: 2 new files + 3 minimal doc pointer additions. The doc's every quantitative claim is generated by the deterministic `scripts/perinatal_feasibility_analysis.py` and carries an inline provenance pointer to it (§9-#2 / L6) — no hand-invented numbers. SMOKE = the script reproduces byte-identically on re-run + the structural finding holds across ≥3 representative years (year-invariance evidence, not a single-year assertion — L7/§9-#3).

### Halt conditions tripped
None. Additive docs+script; zero canonical-state mutation ⇒ §7-#5/#6/#18 not in play. The §7.13/Sub-Q42 finding is RESOLVED by the user-authorized re-scope (DECISION_LOG 2026-05-23T08:00:00Z) — do NOT re-halt; a NEW §7 halt only if the multi-year SMOKE somehow contradicted the 2020 structural finding (it cannot — the blockers are year-invariant; if a year showed ≥5% it would be a NEW §7 finding to surface, not silently absorb).

### Result
**PROCEED** to SMOKE (deterministic multi-year re-run reproducibility + year-invariance) → DO (script + doc + 3 pointer additions; tag `C8.19-pre-do` first) → VERIFY (git scope additive-only; 11-gate SHAs byte-exact; script bit-identical; pytest 347P+1S+1XF unaffected; every doc numeric traces to the script; cross-links resolve) → RECEIPT + STATUS + DECISION_LOG-execution-note + commit + tag `C8.19-complete`.
...
2. **Manuscript Coverage re-paragraph DEFERRED to D.4 (Phase D) — zero `paper/draft_v2_hmd_styled.md` edit.** Per KICKOFF "Manuscript Coverage paragraph re-paragraphed ONCE at D.4 after all data lands" + the C8.17 step7 precedent (manuscript NOT edited; D.4-deferred) + STATUS "Carried-forward (Phase D): manuscript Coverage re-paragraph = D.4". C8.19 (perinatal-record) still pending → "all data" not landed; re-paragraphing now then again at C8.19 then again at D.4 is exactly what KICKOFF forbids. §15.D DO-step-7 "manuscript references" = the DOI/citation references, unchanged by C8.18 (no unified-HVS DOI yet — Phase D.2). Recorded as a Phase-D forward-HALT (the manuscript-D.4 must carry natality 1968-2024 + linked v4 1983-2023 + the 4 framing notes).
3. **`file_inventory.csv` `imported`-flag refresh + the generated `external_validation_v3_linked_comparison.md` regeneration DEFERRED (not in DO-step-7 docs scope).** `file_inventory.csv` is a metadata CSV with its own `tests/test_inventory_invariants.py` (out of the §15.D named scope; the 6b canonical step deliberately did not flip it — flipping it in DO step 7 = a non-doc metadata mutation widening VERIFY-A blast radius / §7-#17). The validation comparison MD is **script-generated** by `compare_external_targets_v3_linked.py` (the 6b fix-on-contact scoped that validator to the 2005-2023 owned surface) — hand-editing a generated artifact is forbidden. Only the **stale narrative line** in `docs/NCHS_SOURCE_MANIFEST.md` ("the linked v3→v4 re-harmonize is a later C8.18 DO step" — now SHIPPED at 6b) is fixed-on-contact (L11 docs). Both deferrals recorded as soft-flags + Phase-D forward-HALTs (the exact C8.17 step7 residents-only-convenience / metadata-refresh deferral pattern).

### Inputs
- [x] All required input files exist (every scoped doc Read-verified to exist; L1-clean — exact unique old-strings captured for surgical Edits, the C8.17 step7 SMOKE method).
- [x] Upstream task complete in STATUS.md: **C8.18 DO step 6b CLOSED** (STATUS 2026-05-23T02:00:00Z; `RECEIPTS/C8.18_step6b_2026-05-23T02-00-00Z.md`; HEAD `127f101`).
- [x] No stale checkpoints: working tree clean (`git status --porcelain` empty); branch `main`; HEAD `127f101` (the 6b commit, after `1b2c1f8`).

### Environment
- [x] git working directory clean ✓; on `main` ✓.
- [x] `C8.18-pre-do` @ `6632a15` present (rollback anchor); **`C8.18-complete` NOT present** (DO step 7 sets it — forward-HALT #1) ✓.
- [x] pytest baseline (6b receipt VERIFY-E) = **347 passed, 1 skipped, 1 xfailed** via the memory-safe per-dir + `tests/`-per-file split (soft-flag (kk); COUNT is the gate). HALT-9 (pytest) deferred to VERIFY per the C8.17 step7 precedent — **docs-only changes provably cannot perturb the parquet/schema/CSV-reading test surface**: `git ls-files | xargs grep -ln` shows NO test reads any scoped `.md`/`.cff`/`.json`/`.R`; `fetal_death/tests/test_release_smoke.py` only *mentions* README in a docstring (its assertions read fetal-death parquet + the in-process schema regenerator, never a README file) and the fetal-death surface is entirely untouched by C8.18.

### Source documentation / canonical-state preconditions (the 11-gate SHA proof — docs-only precondition)
- [x] **10/11 gate parquet SHAs byte-exact** vs the 6b receipt (`shasum -a 256`, `/tmp/c8_18_s7_gate_sha.txt`): `38e2cecb` (fd harm) · `185c071e` (fd derived) · `c8a740eb` (nat v2 harm) · `acb5c48a` (nat v2 derived) · `230efed2` (nat v2 .v28_baseline harm) · `e16ad532` (nat v2 .v28_baseline derived) · `adbec108` (MM harm) · `5c22308b` (MM 95-97 raw) · `7c682668` (MM 95-00 raw) · `d98b4296` (MM 16-20 raw). ✓
- [x] **The linked-derived gate SHA CHANGED `9b828a4d…` → `f630d8cf…`** — exactly the forward-HALT #2 prediction (the v4 SHA; the ONLY C8.18 canonical mutation, made at 6b). `natality_v3_linked_harmonized.parquet` (v4) = `ea89ab3c…`. ✓ (do NOT "restore" to `9b828a4d`).
- [x] **`.v3_baseline` preserved byte-exact**: `natality_v3_linked_harmonized_derived.v3_baseline.parquet` = `9b828a4d…` (== the pre-6b gate; the 2005-2023 byte-clean anchor; 74,943,824 rows / 94 cols). `…_harmonized.v3_baseline.parquet` = `e1795ac6…` (74,943,824 / 78). ✓
- [x] Parquet mtimes (May 13 / May 18) all predate any DO-step-7 work — cheap no-rebuild proof. ✓
- [x] No NVSR/user-guide PDF SHAs touched (docs-only step; no source-doc dependency).

### Field-value snapshot for cells / rows / columns being mutated (Convention 3)

**Authoritative v4 facts (derived, not invented — L6):** linked product → **v4**; coverage **1983-2023** (41-calendar-year span; **permanent 1992-1994 gap** → **38 years with data**); **149,386,620 rows** (= 74,442,796 pre-2005 cohort + 74,943,824 existing 2005-2023; cross-checked `74442796+74943824==149386620`). Column counts from the actual v4 parquet schemas (`pq.read_schema`): `natality_v3_linked_harmonized.parquet` **78→81** cols; `natality_v3_linked_harmonized_derived.parquet` **94→97** cols (+3 within-era cohort cols: `underlying_cause_icd9`@76, `cause_recode_61`@77, `link_segment`@80); `harmonized_schema.csv` 97 data rows (6b HALT-6(a)-(f), already shipped). The B.12 snapshot `tests/snapshots/v3_2026-05-23T02-00-00Z_columns.csv` row_count = 149,386,620; `EXPECTED_COLUMN_COUNTS["natality_v3_linked_harmonized_derived"]=97` (test pin, 6b-set). `external_validation_targets_v3_linked.csv` = 156 data rows (62 2005-2023 + the pre-2005 cohort cells incl. the 5 documented-tolerance cells from the §7 Option-A).

**The 4 mandated framing notes to carry into the consumer-doc surface (COMPARABILITY/ABOUT/CODEBOOK):** (i) keyless **1983-1988** within-era structural difference — cohort IMR = count(`link_segment`='num') / count(`link_segment`='den') per stratum, NOT a per-birth `infant_death` filter (den rows `infant_death` NULL; num rows True); (ii) **1983-1984** 50%-non-VSCP RECWT-weighted sample — harmonized 1983-1984 uses `record_weight`=RECWT@91 to reproduce published cohort figures byte-exact (1983 Σwt-by-residence 3,639,113 == guide; IMR 10.9046 ≈ 10.9; 1984 3,669,268; 10.4359 ≈ 10.44); (iii) the documented **cohort_num numerator-file-vs-denominator-linkage residual** for 3 of 19 years (1989 Δ+1, 1998 Δ+40, 2002 Δ−8) — the SAME NCHS cohort-linked class as the shipped-v3 Task-6 "33/35 byte-exact + 2 cells differ by exactly 1 within documented tolerance" (extended to 3 older cohort years; pinned `tolerance_abs` + sourced notes at 6b); (iv) **1992-1994 permanent gap** — NCHS suspended ALL linkage; already in `harmonized_schema.csv` `years_available` (6b — verified: `infant_death`/`age_at_death_days` rows read `"1983-2023 (linked; 1992-1994 gap)"`), to be added to CODEBOOK + ABOUT + (D.4-deferred) manuscript per the §15.D VERIFY criterion.

- [x] Target rows / cells / columns enumerated + current values captured (the discriminating principle — **SWAP**: `74,943,824`→`149,386,620`; `2005–2023`/`(19 years)`/`(19 cohort yr)`/`19-yr`→`1983–2023`/`(38 years; permanent 1992–1994 gap)`; linked harmonized `78`→`81`; linked derived `94`→`97`; linked product version `v3`→`v4`; add the 4 framing notes. **PRESERVE**: the 2005-2023 "33/35 byte-exact + 2 differ-by-1 within tolerance" framing (still accurate for that surface, unchanged by v4 — add the pre-2005 framing alongside, do not overwrite); the canonical parquet FILENAME `natality_v3_linked_*` + script names `harmonize_linked_v3.py`/`derive_linked_v3.py` (the "v3" is the linked schema-family tag, retained per Anti-Pattern #6 / the C8.17 `.v28_baseline` precedent — do NOT rename to `v4`); historically-correct refs (CHANGELOG [v1.0] "Linked v3 — 19 years (2005–2023)"; v2.7.0-latest-deposited framing); the linked `residents_only` convenience parquet 74,785,708/92 (NOT rebuilt for v4 — annotate as v3-2005-2023-slice / v4-convenience-refresh-pending, the C8.17 natality-residents_only precedent; do NOT fabricate a v4 number); reproduce-command blocks updated to the v4 envelope (`parse_all_linked_years.py` 1983-1991 + 1995-2004 + 2005-2015 + the 2016-2023 period-cohort loop; `harmonize_linked_v3.py` default `--years 1983-2023` gap-auto-skipped; `derive_linked_v3.py`) — verified canonical via the harmonize_linked_v3.py docstring + parse_all_linked_years.py `_zip_name_for_year` (L1-clean, not invented)).
- [x] Current values match task plan's assumed state ✓ — the only divergences are the three scope decisions above, all resolved at this cheap-check moment via the C8.17 step7 precedent + standing authorization + the plan-mandated D.4 deferral (documented; not silently proceeded).

### Outputs
- [x] No new binary artifacts. Git-tracked docs MODIFIED only; intended edits are surgical string swaps + the 4 framing-note insertions + the CHANGELOG C8.18 entry + this PRE-FLIGHT entry.

### Halt conditions tripped
None. The 6b §7 cohort_num finding is RESOLVED (Option A; do NOT re-halt — forward-HALT #5). The scope-breadth / manuscript-D.4 / metadata-CSV decisions are precedented (C8.17 step7) + standing-authorized + plan-mandated, NOT §7-#17 trips. Docs-only ⇒ no canonical-state mutation ⇒ §7-#5/#6/#18 not in play.

### Result
**PROCEED** to SMOKE (per-file exact old→new enumeration + the discriminating principle) → DO (docs/metadata consumer-surface sync, docs-only) → VERIFY (A git-scope docs-only + 11-gate unchanged; B 1992-1994 gap in schema+CODEBOOK+ABOUT [manuscript=D.4-deferred]; C pytest 347P+1S+1XF memory-safe split; D residual stale-string audit; E version/framing internal consistency) → RECEIPT + STATUS + commit + **tag `C8.18-complete`** (the FINAL C8.18 sub-step → unblocks C8.19).

---

## PRE-FLIGHT CONTINUATION for C8.18 DO step 6b — 2026-05-23T01:00:00Z — the gate is CLEARING (the established 5c-iii 02:00:00Z / 6a 12:00:00Z dated-continuation precedent; L10-safe — this continuation precedes the first DO mutation; the DO does NOT begin until all three gates clear) — **RESULT so far: gate (i) parse-complete ✓; gate (ii) H6-verify-all-19 byte-exact ✓ (the §7 gate per forward-HALT #7 — CLEARED, no divergence, NO §7 halt); gate (iii) pytest 338P+1S+1XF baseline — LAUNCHED (background `bgyr4oygt`), DO proceeds when it confirms**

### Gate (i) — the single-clean-writer background parse COMPLETE ✓

The STATUS-tracked background parse (PID 43935, `--years 1984-2004`, started Mon 2026-05-18 13:59:42, continuous/uninterrupted single sequential clean writer) **exited cleanly** (the watcher fired; parent gone). Parse-log summary: `Total: 71,061,818 rows across 18 years` (1984-2004; 1992-1994 SKIP). 2003 = the documented DEFLATE64 long pole: 4,096,151 rows in 11,093.0s (~3h05m); 2004 = 4,118,956 rows in 4,546.9s (~76min, DEFLATE). **All 19 cohort `_raw` parquets present** (1983 separate — the 6a-RECWT VERIFY-D byte-exact one; 1984-2004 from this run). The wait-not-kill decision (main entry HALT #7) is validated: a single clean continuous pass produced the substrate; the H6-verify (below) is the trust gate, not a redundant re-parse.

### Gate (ii) — H6-verify-all-19 byte-exact ✓ (the §7 gate; forward-HALT #7) — CLEARED

`/tmp/c8_18_s6/h6_verify_all19.py` (replicates the 6a VERIFY-D method; reads the L9-sourced `cohort_den/num_file_records` control totals state-on-disk from `external_validation_targets_v3_linked.csv`; counts parsed `_raw` rows split by `link_segment` for the keyless 1983-1988, `ParquetFile.metadata.num_rows` for the 1989+ denominator-plus). **ALL 19 cohort years byte-exact (Δ=0), den all 19 + num 1983-1988:**

| yr | den parsed == guide | yr | den parsed == guide |
|---|---|---|---|
| 1983 | 3,341,274 ✓ (num 39,704 ✓) | 1997 | 3,884,329 ✓ |
| 1984 | 3,364,090 ✓ (num 38,314 ✓) | 1998 | 3,945,192 ✓ |
| 1985 | 3,765,336 ✓ (num 39,170 ✓) | **1999** | **3,963,465 ✓** |
| 1986 | 3,760,997 ✓ (num 37,966 ✓) | **2000** | **4,063,913 ✓** |
| 1987 | 3,813,492 ✓ (num 37,349 ✓) | **2001** | **4,031,646 ✓** |
| 1988 | 3,913,967 ✓ (num 37,599 ✓) | **2002** | **4,027,475 ✓** |
| 1989 | 4,045,881 ✓ | **2003** | **4,096,151 ✓** (DEFLATE64) |
| 1990 | 4,163,150 ✓ | **2004** | **4,118,956 ✓** |
| 1991 | 4,115,494 ✓ | 1995 | 3,903,012 ✓ |
| 1996 | 3,894,874 ✓ | | |

The 6 bolded years (1999-2004) are the ones 6a deferred ("finishing in background; re-verified at 6b") — now byte-exact, **including the DEFLATE64 2003**. Zero silent drops; the §7-fixed chunked `run_parse` is lossless at full scale across all 19 (the 5b "Lossless" claim now fully proven end-to-end). **No divergence → forward-HALT #7 / #10 do NOT fire; the §7/§8 finding stays RESOLVED (no re-halt). §7 gate CLEARED.**

### Gate (iii) — pytest 338P+1S+1XF pre-DO baseline — LAUNCHED

Background `bgyr4oygt`: the 4-dir suite (`fetal_death/tests/ natality/tests/ tests/ matched_multiples/tests/`) `--ignore` the intentionally-RED `test_linked_6b_reharmonize_smoke.py` (SMOKE-first §9-#9 — its 4 Tier-0 RED-pre-DO failures are by design; the pre-DO baseline is the 6a-RECWT VERIFY-E close state = 338 passed, 1 skipped, 1 xfailed). The DO does NOT begin until this confirms 338P+1S+1XF (no pre-existing regression attributable elsewhere; ~1171s last VERIFY — soft-flag (kk) advisory; the COUNT is the gate).

### SMOKE-first (§9-#9) state-on-disk

`natality/tests/test_linked_6b_reharmonize_smoke.py` authored (`DESIGN: tracks-current-state`, SHAPE-not-VALUE) BEFORE any DO; Tier-0 confirmed **RED 4-fail + 1-pass** (`_LINKAGE_GAP` ImportError; schema-completeness; new-row dtype; widened death-side years; + the 2005+ canonical-path §9-#7 regression-lock PASS). The DO turns the 4 RED → GREEN.

### Result

**Gates (i)+(ii) CLEARED.** H6-verify-all-19 byte-exact (the binding §7 gate — no divergence, no halt). **PROCEED to DO the moment gate (iii) (the pytest 338P+1S+1XF pre-DO baseline, background `bgyr4oygt`) confirms.** L10-safe — this continuation precedes the first DO mutation; the DO (re-harmonize → v4 + HALT-6(a)-(f) schema edits + v3→v4 bump + L17 re-pins) begins only after gate (iii). A NEW §7 halt is required ONLY if the post-DO full §15.D per-year VERIFY shows the weighted 1983-1984 (or any cohort year) diverging beyond tolerance (forward-HALT #10).

---

## PRE-FLIGHT for C8.18 DO step 6b — 2026-05-23T00:00:00Z — the **FULL canonical re-harmonize linked 1983-2023 → v4** (the FIRST canonical parquet/`harmonized_schema.csv` mutation in C8.18; Anti-Pattern #6 version bump) — **RESULT: PROCEED to SMOKE (Tier-0 author-first, parse-independent §9-#9); the DO (re-harmonize + schema edit + version bump) is GATED on three pending checks clearing — (i) the single-clean-writer background parse completing 2003 (DEFLATE64 long pole, actively in progress) + 2004, (ii) the H6-verify-all-19 byte-exact gate (forward-HALT #7; the actual trust mechanism — §7 halt if any year diverges), (iii) the pytest 338P+1S+1XF baseline re-confirmed post-parse**. The cheap PRE-FLIGHT checks (git anchors, 11/11 gate parquet SHAs byte-exact, Convention-3 Field-value snapshot, HALT-6(a)-(f) enumeration) all PASS. The H6-verify-all-19 gate is recorded here as PENDING-not-cleared with a dated PRE-FLIGHT continuation owed when the parse finishes (the established 5c-iii 02:00:00Z / 6a 12:00:00Z dated-addendum precedent; L10-safe — this entry precedes the first DO mutation, and the DO does not begin until the gate clears).

### Entry cheap-check — 10 forward-looking HALTs (from `RECEIPTS/C8.18_step6a-RECWT_2026-05-22T02-00-00Z.md`)

1. **Git anchors** ✓ — `C8.18-pre-do`@`6632a15` present; `C8.18-complete` NOT present; `C8.16-complete`/`C8.17-complete` present. HEAD = `1b2c1f8` (the 6a-RECWT commit, after `2a348aa` the 6a commit). Working tree clean on `main`.
2. **11 gate parquet SHAs byte-exact** ✓ — recomputed (sorted, first-8): `185c071e 230efed2 38e2cecb 5c22308b 7c682668 9b828a4d acb5c48a adbec108 c8a740eb d98b4296 e16ad532` == the 6a-RECWT receipt VERIFY-A set exactly (zero canonical mutation since 6a-RECWT). The linked-derived `9b828a4d…` changes ONLY at THIS sub-step's 6b re-harmonize (by design — 6b is the first canonical mutation).
3. **pytest baseline = `338 passed, 1 skipped, 1 xfailed`** (4-dir; the 6a-RECWT VERIFY-E gate). Re-confirmed post-parse, pre-DO (the established per-sub-step pattern; ~1171s last VERIFY — soft-flag (kk) advisory; the COUNT is the gate). 6b re-baselines after the L17 re-pins (count-shape: the row-count-conservation LINKED pins are tracking-state, count-neutral; the B.12 sibling snapshot is a new CSV asset not a test count change).
4. **`_harmonize_cohort_1983_1988` populates `record_weight` from RECWT@91 for year∈{1983,1984}** (era-gated; 1985-1988 NULL) — state-on-disk; do NOT re-open. `field_specs.py` `("RECWT",91,91)` CONFIRMED correct (do NOT "re-derive"); the 1988 trailing-byte anomaly = soft-flag (ll) carry (1985-1988 unweighted == published-comparable; NOT 1983-2023-IMR-blocking).
5. **The 5c-iii `record_weight`=NULL is SUPERSEDED for 1983-1984** (DECISION_LOG 2026-05-21T12:00:00Z + 2026-05-22T02:00:00Z); correct for 1985-1988. Do NOT re-apply NULL for 1983-1984 at 6b — the re-harmonize MUST carry the 6a-RECWT era-gated populate into the v4 parquet (the durable end-to-end gate for the 6a-RECWT fix = this sub-step's §15.D weighted-1983-1984 VERIFY).
6. **DO step 6b = the FULL canonical re-harmonize → v4** — this PRE-FLIGHT. The mandatory `harmonized_schema.csv` HALT-6(a)-(f) enumeration is below (the PRE-FLIGHT halts if it does not enumerate; it does — see "HALT-6(a)-(f) enumeration").
7. **2003+2004 `_raw`**: the STATUS-tracked single-clean-writer background parse (PID 43935→child 43939, `--years 1984-2004`, started Mon 2026-05-18 13:59:42, continuous/uninterrupted) is ACTIVELY parsing **2003** (child PID 48299 = `7z x -so LinkCO03US.zip VS03LKBC.USDENPUB`, the documented DEFLATE64 long pole; `linked_2003_denomplus.parquet` growing, fd 4w open; ~2h53m on the 7z stream); 2004 not yet started; all other 17 years (1983 separate + 1984-2002 from this run) present. **Decision: WAIT, do NOT kill+restart** — §2 cheap-before-expensive (killing discards ~3.5h of a correct continuous single-clean-writer pass and re-incurs the entire 2003 DEFLATE64 cost for zero integrity gain) + forward-HALT #7's intent is satisfied by the **H6-verify-all-19 byte-exact gate** (parsed den/num record count == guide File "Record count" in `external_validation_targets_v3_linked.csv`, byte-exact, ALL 19) which is the definitive trust mechanism regardless of which clean run produced the `_raw`. The "do NOT trust a possibly-interrupted background `_raw`" concern is discharged BY the H6-verify (not by a redundant re-parse). If any year fails H6 byte-exact → re-parse that year cleanly; if still divergent → §7 halt.
8. **soft-flag (ll) carries** (RECWT-location resolved at 6a-RECWT; the per-year 1985-1988 non-anchor layout reconstruction = 3a-class future, NOT 1983-2023-IMR-blocking). (gg) carries (composite-block leaf decomposition → 6b/post-C8.18; stays conservatively NULL). (ii) §11 human-merge now also folds the 6b model note. (kk)/(jj)/(hh)/(cc)/(dd)/(ee)/(bb)/(aa)/(w)/(x)/(z)/(u) carry.
9. **L17 grep-scope** ✓ — ALL 13 cohort/linked harnesses scanned for v3/v4/`OUT_SCHEMA`-count/`harmonized_schema`-row/year-coverage pins. The cohort SMOKE harnesses assert `schema == OUT_SCHEMA` **relatively** (no col-count literal — robust to 80→81). The genuine tracking-state pins that 6b MUST re-pin SAME commit: `tests/test_row_count_conservation.py` (`LINKED_EXPECTED_TOTAL = 74_943_824`, `LINKED_EXPECTED_YEARS = list(range(2005, 2024))`, the "v3); 19 yrs 2005-2023" comment — all `DESIGN: tracks-current-state`) + the B.12 snapshot (`tests/snapshots/v2_2026-05-16T08-00-00Z_columns.csv` → a NEW sibling `v3_<UTC>_columns.csv` via `tests/snapshots/_build_snapshot.py` + a DECISION_LOG entry naming the cause, per `test_parquet_column_snapshot.py`'s docstring re-snapshot rule).
10. **The §7/§8 finding is RESOLVED** (surfaced + halt-and-asked at 6a → Option-A → executed + verified byte-exact at 6a-RECWT). Do NOT re-halt it. A NEW §7 halt is required ONLY if the 6b full §15.D per-year VERIFY shows the WEIGHTED 1983-1984 (or any cohort year) diverging from the NCHS-published cohort figures beyond tolerance (forward-HALT #10 honored).

### HALT-6(a)-(f) enumeration (MANDATORY per 5c-iii forward-HALT #6 + 6a-RECWT forward-HALT #6 — the PRE-FLIGHT halts if it does not enumerate; the precise cell wording is the DO act, the plan is enumerated here)

Convention-3 Field-value snapshot of `natality/metadata/harmonized_schema.csv` (95 data rows; the linked-only death-side rows are 86-95; certificate_revision row 5 already `1968-2024` / `unrevised_1968|unrevised_1989|revised_2003|unknown` from C8.17 DO step 6 — accommodates cohort 1983-1988 → unrevised_1968 with NO row-5 edit). Current vs planned:

- **(a) widen the death-side rows' `years_available` 2005-2023 → 1983-2023 (1992-1994 gap)** — rows 86 `infant_death`, 87 `age_at_death_days`, 88 `age_at_death_recode5`, 89 `underlying_cause_icd10`, 90 `cause_recode_130`, 91 `manner_of_death`, 92 `record_weight`, 93 `neonatal_death`, 94 `postneonatal_death`, 95 `cause_group` all currently `2005-2023 (linked)`. Plan: widen to `1983-2023 (linked; 1992-1994 gap)` WITH per-row null-pattern nuance — `age_at_death_days` carries NO 1983-1988 value (AGER5-only; the 1983-1988 num has only `age_at_death_recode5`); `underlying_cause_icd10`/`cause_recode_130` null 1983-1998 (ICD-9 era — couples to (b)); `manner_of_death` null 1983-2002 (couples to (c)); `record_weight` non-null 1983-1984+2005-2023, null 1985-2002 (the 6a-RECWT note — couples to (e)); `infant_death` carries the within-era structural note (1983-1988 = den NULL / num True; IMR = count(num)/count(den), NOT a per-birth filter).
- **(b) ADD `underlying_cause_icd9` + `cause_recode_61` rows** (NEW — absent from the schema CSV; present in OUT_SCHEMA lines 328-329). Types MUST match OUT_SCHEMA pyarrow dtypes for the C8.1 dtype-parity test: `underlying_cause_icd9` → `string` (OUT_SCHEMA `pa.string()`), `cause_recode_61` → `int16` (OUT_SCHEMA `pa.int16()`). years_available `1983-1998 (linked cohort ICD-9 era; 1992-1994 gap)`; null 1999+ (the ICD-9≤1998 / ICD-10≥1999 default-null + revision-tagged note per DECISION_LOG 2026-05-17T05:30:00Z).
- **(c) widen `manner_of_death` (row 91) `years_available`** — currently `2005-2023 (linked)`. Plan: cohort 2003-2004 + 2005-2023; null 1983-2002 (incl. 1983-1988 — the 1983-1988 num carries no MANNER; 2003-2004 carries MANNER per 5c-ii-b).
- **(d) widen `father_age_cat_from_rec11` (row 39) `years_available`** — currently `2005-2013` (within-era; FAGEREC11). Plan: add cohort 2003-2004 (the 5c-ii-b 2003-2004 cohort birth-side derives father-age-cat from REC11). Final `2003-2013` (cohort 2003-2004 + 2005-2013; null 1990-2002 + 2014+).
- **(e) ADD the `link_segment` schema row** (NEW — absent from the schema CSV; OUT_SCHEMA line 347 `pa.string()`). Type → `string`. years_available `1983-1988 (linked cohort keyless era)`; null every other era (single denominator-plus / 2005+). Notes MUST carry: the within-era structural-difference (1983-1988 IMR = count(link_segment="num")/count(link_segment="den") per stratum — NOT a per-birth `infant_death` filter; the D.4 Coverage item) + the 1983-1988 `record_weight` non-null-for-1983-1984-only / hispanic-NULL pattern + the "no `age_at_death_days`, `age_at_death_recode5`-only" 1983-1988 note + cert_rev `unrevised_1968` for 1983-1988 (row 5 already covers 1968-1988→unrevised_1968 — cross-reference, no row-5 edit). PLUS the 6a-RECWT note on row 92 `record_weight`: `1983-1984` populated from RECWT@91 (50%-non-VSCP weighted sample) + `2005-2023`; null 1985-2002 (incl. 1985-1988 full-file, no weighting; the 1988 trailing-byte anomaly soft-flag (ll)).
- **(f) bump linked v3 → v4** — Anti-Pattern #6 (schema edit paired with the version bump + the DECISION_LOG reference; the natality C8.17 precedent embedded the bump rationale in the row-5 `notes` + a DECISION_LOG ref). The linked logical version v3→v4 recorded in the relevant linked schema row notes + a DECISION_LOG 2026-05-23 entry; the canonical parquet FILENAME `natality_v3_linked_harmonized_derived.parquet` is RETAINED (the "v3" there is the linked schema-family tag, exactly the natality `.v28_baseline`/`natality_v2_*` precedent where v3.0.0 kept the `natality_v2_*` filename); the prior derived parquet preserved as `natality_v3_linked_harmonized_derived.v3_baseline.parquet` (the forward-stability anchor for the 2005-2023 byte-clean regression VERIFY). [Architectural decision — filename retention + gap-skip in `harmonize_linked_v3.main()` + default `--years`/`--out` for canonical-command reproducibility — RESOLVED at DO with a DECISION_LOG entry; flagged here, not silently deviated.]

### Inputs

- [x] `RECEIPTS/C8.18_step6a-RECWT_2026-05-22T02-00-00Z.md` (the 10 forward-HALTs; the §7/§8-resolved disposition) — read.
- [x] `RECEIPTS/C8.18_step5c-iii_2026-05-20T02-00-00Z.md` (the HALT-6(a)-(f) full definition; the keyless 1983-1988 model) — read.
- [x] `natality/scripts/03_harmonize/harmonize_linked_v3.py` (2268L; `OUT_SCHEMA` 81 cols incl. `link_segment` last + `underlying_cause_icd9`/`cause_recode_61`; `_cohort_era`; `_harmonize_cohort_*`; `main()` default `--out natality_v3_linked_harmonized.parquet` / `--years 2005-2023`) — read.
- [x] `natality/scripts/04_derive/derive_linked_v3.py` (266L; schema-driven `out_fields = list(in_schema) + [16 derived]` — auto-carries the new OUT_SCHEMA cols; default in/out `natality_v3_linked_harmonized[_derived].parquet`) — read.
- [x] `natality/scripts/01_import/parse_all_linked_years.py` (135L; `_zip_name_for_year` era-keyed; deterministic full re-parse per year, no skip; 1992-1994 SKIP; 2016+ out-of-driver) — read.
- [x] `natality/metadata/harmonized_schema.csv` (95 rows; HALT-6 Field-value snapshot above) + `natality/metadata/external_validation_targets_v3_linked.csv` (157 rows; the 19-year `cohort_den/num_file_records` H6 control totals + `resident_births`/`resident_infant_deaths`/`imr_per_1000` incl. the weighted 1983-1984 — state-on-disk, do NOT re-source) — read.
- [x] `tests/test_row_count_conservation.py` (LINKED pins, lines 45-49/177-220, `DESIGN: tracks-current-state`) + `tests/test_parquet_column_snapshot.py` + `tests/snapshots/_build_snapshot.py` + `tests/snapshots/v2_2026-05-16T08-00-00Z_columns.csv` (B.12 re-snap rule) + `natality/tests/test_schema_dtype_parity.py` (covers linked-derived 94 cols — the new HALT-6(b)/(e) rows' `type` must match OUT_SCHEMA) — read.
- [x] 11 gate parquets — SHA byte-exact (above).
- [ ] **PENDING (gate)**: the 19 pre-2005 cohort `_raw` parquets — 1983 (6a-RECWT VERIFY-D byte-exact) + 1984-2002 (this background run) present; **2003 in progress (DEFLATE64), 2004 not started**. H6-verify-all-19 NOT yet run — DO blocked until it clears.

### Environment

- [x] `uv run python` / `pyarrow` / `pyarrow.compute` / `pyarrow.parquet` available. Working dir clean; branch `main`. The background parse (PID 43935) is a pre-existing STATUS-tracked process, not launched this session.

### Source documentation

- [x] The 19 cohort guide PDFs SHA-anchored at C8.18 DO step 2 (`docs/NCHS_SOURCE_MANIFEST.md` §3 / `natality/metadata/file_inventory.csv`); the H6 control totals + the weighted 1983-1984 targets were L9-sourced + state-on-disk at DO step 6a / 6a-RECWT in `external_validation_targets_v3_linked.csv` (do NOT re-source).

### Outputs (intended — at DO, after the gate clears)

- `natality/scripts/03_harmonize/harmonize_linked_v3.py` — `main()` gap-skip (1992-1994, mirroring `parse_all_linked_years` + `_cohort_era` §2 fail-closed) + default `--years 1983-2023` + docstring; behaviour-preserving for 2005-2023 (the `_cohort_era is None` byte-identity §9-#7).
- `natality/scripts/04_derive/derive_linked_v3.py` — schema-driven; no logic change expected (auto-carries link_segment/icd9/cause_recode_61). Default paths retained.
- Canonical re-harmonized parquets: `natality_v3_linked_harmonized.parquet` (intermediate) + `natality_v3_linked_harmonized_derived.parquet` (SHIPPED canonical; v3-filename retained, logical v4) — the linked-derived gate SHA `9b828a4d…` CHANGES (by design — the only canonical mutation in C8.18). Prior derived preserved as `natality_v3_linked_harmonized_derived.v3_baseline.parquet` (+ harmonized `.v3_baseline` for the column-level 2005-2023 regression).
- `natality/metadata/harmonized_schema.csv` — HALT-6(a)-(f) edits (Anti-Pattern #6: paired with the v3→v4 bump + DECISION_LOG 2026-05-23 reference).
- `tests/test_row_count_conservation.py` — L17 `tracks-current-state` re-pin (LINKED_EXPECTED_TOTAL/_YEARS/comment) SAME commit.
- `tests/snapshots/v3_<UTC>_columns.csv` (NEW sibling via `_build_snapshot.py`) + the DECISION_LOG re-snap cause entry SAME commit (B.12).
- `PRE_FLIGHT_LOG.md` (this + a dated continuation when the parse completes/H6 runs) + `RECEIPTS/C8.18_step6b_…` + `DECISION_LOG.md` 2026-05-23 + `STATUS.md` — appended.
- **No git tag** (intermediate DO step; `C8.18-pre-do`@`6632a15` rollback anchor; `C8.18-complete` is DO-step-7-only).

### Field-value snapshot for cells / rows / columns being mutated (Convention 3)

| Artifact / row | Task-plan-assumed state | Actual verified state | Resolution |
|---|---|---|---|
| 11 gate parquet SHAs | unchanged since 6a-RECWT | 11/11 byte-exact (above) | ✓ no divergence |
| `harmonized_schema.csv` death-side rows 86-95 `years_available` | `2005-2023 (linked)` (pre-6b) | confirmed `2005-2023 (linked)` all 10 | MUTATE per HALT-6(a)/(c) at DO |
| `harmonized_schema.csv` `underlying_cause_icd9`/`cause_recode_61`/`link_segment` rows | absent (DO step 6b adds) | confirmed ABSENT (rows present in OUT_SCHEMA only) | ADD per HALT-6(b)/(e) at DO; types string/int16/string == OUT_SCHEMA |
| `harmonized_schema.csv` row 5 `certificate_revision` | already `1968-2024` / incl. `unrevised_1968` (C8.17) | confirmed `1968-2024`, `unrevised_1968|unrevised_1989|revised_2003|unknown`, notes name 1968-1988→unrevised_1968 | NO row-5 edit (already accommodates cohort 1983-1988); cross-reference in (e) |
| `harmonized_schema.csv` row 39 `father_age_cat_from_rec11` `years_available`=`2005-2013`; row 91 `manner_of_death`=`2005-2023 (linked)` | pre-6b | confirmed | MUTATE per HALT-6(d)/(c) at DO |
| `test_row_count_conservation.py` `LINKED_EXPECTED_TOTAL=74_943_824` / `LINKED_EXPECTED_YEARS=range(2005,2024)` | pre-6b (2005-2023 only) | confirmed (lines 48-49) | L17 `tracks-current-state` re-pin at DO (new total from the v4 §15.D VERIFY; years = gap-aware 1983-2023) |
| `tests/snapshots/v2_2026-05-16T08-00-00Z_columns.csv` | the linked-parquet rows reflect v3 2005-2023 | confirmed (latest baseline) | B.12 re-snap → sibling `v3_<UTC>_columns.csv` + DECISION_LOG cause at DO |
| 19 pre-2005 `_raw` den/num record counts vs guide `cohort_den/num_file_records` | byte-exact (H6 — the 6a parser-conservation gate, re-verified at 6b per forward-HALT #7) | **PENDING** — parse in progress (2003 DEFLATE64; 2004 not started) | H6-verify-all-19 at the dated PRE-FLIGHT continuation post-parse; §7 halt if any year diverges |

### Halt conditions tripped

None at this PRE-FLIGHT. The cheap checks PASS. The H6-verify-all-19 + pytest-baseline are PENDING (gated on the in-progress single-clean-writer parse) — recorded as NOT-cleared; the DO does not begin until they clear; a §7 halt fires only if H6 diverges (forward-HALT #7) or the full §15.D per-year VERIFY diverges (forward-HALT #10). The wait-not-kill decision (HALT #7 above) is a §2 cheap-before-expensive call, not a deviation. No §7-#17 scope-creep (the HALT-6(a)-(f) edits + L17 re-pins + .v3_baseline are exactly the §15.D DO step 6b declared scope).

### Result

**PROCEED** to SMOKE (§9-#9 SMOKE-first; the Tier-0 layout/dispatch/structural assertions are parse-independent and authored now, RED→GREEN). The **DO (re-harmonize → v4 + schema HALT-6 + version bump + L17 re-pins) is GATED**: it does NOT begin until (i) the parse completes 2003+2004, (ii) H6-verify-all-19 is byte-exact (§7 halt if not), (iii) the pytest 338P+1S+1XF baseline is re-confirmed. A dated PRE-FLIGHT continuation records the gate clearing (L10-safe — this entry precedes the first DO mutation; the continuation is the established C8.18 dated-addendum pattern, not a back-fill).

---

## PRE-FLIGHT for C8.18 DO step 6a-RECWT — 2026-05-22T00:00:00Z — the **bounded 1983-1984 RECWT root-cause fix** (executing the §7/§8-satisfied Option-A from the 6a addendum) — **RESULT: PROCEED to SMOKE/DO with a REFINED (smaller) scope**: the re-derivation FALSIFIED the task-plan-assumed defect — the 3a `LINKED_BIRTH_1983_1988_FIELDS` `("RECWT", 91, 91)` byte position is **CORRECT** (LinkCO83Guide.pdf p13 coordinate-aligned "List of Data Elements and Locations" places `1.f Record weight` at Denominator byte **91**; empirically Σ(RECWT@91) over the full parsed 1983 den == guide "By occurrence" 3,643,001 **byte-exact**, 1984 3,673,694 vs 3,673,693 off-by-1) → the 5c-iii **(A′) "RECWT@91 is era-unstable/wrong" conclusion was an over-conservative head-sample artifact** (the denominator file is ordered by State-of-occurrence per p4 → the first 5000 records are all VSCP-area weight-1 records, and `DMRACE@57`≈"1"=white dominates, so the head-sample `byte91==byte57` coincidence is 75-81% on the FULL file for 1983-1987, NOT the claimed 100%; 1988 IS a genuine trailing-byte anomaly — 100% byte91==byte57, multi-valued — but 1988 is a full file [Record count == by-occurrence] needing NO weighting, so NULL stays correct there); therefore **NO `field_specs.py` position edit and NO re-parse are needed** — the fix is **harmonize-only** in `_harmonize_cohort_1983_1988` (populate `record_weight` from RECWT@91 for year ∈ {1983,1984}; 1985-1988 stay NULL — the §7-scoped 1983-1984-only decision is exactly right) + the bundled L17 `tracks-current-state` reframe of the now-stale 5c-iii harness `record_weight==[None]` pins (§4.2.1/Convention-2). This is a within-task cheap-check scope refinement (the 5a "7z-stream-already-wired" / 4a/4b "prior assumption FALSIFIED" precedent), NOT a new §7 trip; the §7/§8 finding + Option-A disposition stand and are EXECUTED (the 1983-1984 IMR IS biased without weighting; the fix is simply smaller than the 6a forward-HALT-#4 plan anticipated).

### Entry cheap-check — 10 forward-looking HALTs (from `RECEIPTS/C8.18_step6a_2026-05-21T12-00-00Z.md`)

1. **Git anchors** ✓ — `C8.18-pre-do`@`6632a15` present; `C8.18-complete` NOT present; `C8.16-complete`/`C8.17-complete` present. HEAD = `2a348aa` (the DO step 6a commit, after `4cb5a2c` the 5c-iii commit). Working tree clean on `main`.
2. **11 gate parquet SHAs byte-exact** ✓ — recomputed (sorted, first-8): `185c071e 230efed2 38e2cecb 5c22308b 7c682668 9b828a4d acb5c48a adbec108 c8a740eb d98b4296 e16ad532` == the 6a receipt set exactly (zero canonical mutation since 6a). Linked-derived `9b828a4d…` changes ONLY at the 6b re-harmonize.
3. **pytest baseline** — state-on-disk `/tmp/c8_18_s6/pytest_baseline.out` = `303 passed, 1 skipped, 1 xfailed in 401.62s`; the committed 6a `test_linked_do6a_parse_smoke.py` (+17) → expected ~320P+1S+1XF. The COUNT is the gate; re-established at this sub-step's VERIFY (full 4-dir suite re-run post-DO; the established per-sub-step pattern — not re-run at PRE-FLIGHT, ~400s).
4. **6a-RECWT scope** — its own PRE-FLIGHT (this). Plan = re-derive correct 1983-1984 RECWT byte (forward-HALT #4a) → **FALSIFIED: @91 is correct** (see header + Field-value snapshot); soft-flag (ll) per-year 1983-1984 (+ 1985-1988 spot) non-anchor re-verification (#4b) → DONE (see Field-value snapshot); `field_specs.py` corrective edit (#4c) → **NOT NEEDED** (position correct); reopen `_harmonize_cohort_1983_1988` `record_weight` (#4d) → the actual fix; re-parse 1983-1984 (#4e) → **NOT NEEDED** (RECWT@91 unchanged → `_raw` byte-identical; already carries correct RECWT@91, empirically used to compute Σ==by-occurrence); WEIGHTED-VERIFY vs guide by-occurrence/by-residence + published IMR (#4f) → the VERIFY criterion (already cross-checked at PRE-FLIGHT: 1983/1984 byte-exact / within-tol — see Field-value snapshot).
5. **5c-iii `record_weight`=NULL SUPERSEDED for 1983-1984** ✓ (DECISION_LOG 2026-05-21T12:00:00Z) — this sub-step EXECUTES it; remains correct for 1985-1988 (full files; 1988 trailing-byte anomaly real but harmless — no weighting needed).
6. **H6 gate + L9 control totals state-on-disk** ✓ — `external_validation_targets_v3_linked.csv` carries the 1983-2004 `cohort_den/num_file_records` + `resident_births`/`resident_infant_deaths`/`imr_per_1000` rows (62→157 lines; do NOT re-source). 1999-2002 `_raw` present + 1983-1991/1995-2002 present; **2003+2004 `_raw` MISSING** (the 6a background parse log ended mid-`VS03LKBC.USDENPUB`) — out of 6a-RECWT scope (1983-1984 only) but flagged for 6b; the idempotent parse re-launched in background this session.
7. **DO step 6b is the next sub-step** (after 6a-RECWT) — the v3→v4 re-harmonize + `harmonized_schema.csv` HALT-6(a)-(f) + the full §15.D VERIFY + D.4. Not in 6a-RECWT scope.
8. **soft-flag (ll) being consumed at 6a-RECWT** ✓ (the RECWT-location re-derivation + the per-year 1983-1984 + 1985-1988 spot non-anchor re-verification — DONE this PRE-FLIGHT; resolves the (A′) finding: @91 correct for 1983-1987, 1988 trailing-byte anomaly documented + harmless). (gg) carries; (ii) §11 human-merge now also folds the (A′)-FALSIFIED correction.
9. **L17** ✓ — grep-scope of all 12 cohort/linked harnesses for `record_weight`: the stale `==[None]`/`all(w is None …)` 1983-1984 pins are in `test_linked_cohort_5ciii_harmonize_smoke.py` ONLY (lines 265, 318, 614-617; `DESIGN: tracks-current-state`). 5cii_a/5cii_b/5b record_weight refs are 1995-2004 era (unaffected). Bundle the minimal `tracks-current-state` Edit SAME commit (§4.2.1/Convention-2/L17).
10. **§7/§8 halt-and-ask satisfied at 6a** ✓ — do NOT re-halt the same finding; 6a-RECWT EXECUTES Option A. A NEW §7 halt is required ONLY if the corrected weighted figures still diverge from published — they do NOT (1983/1984 reconcile byte-exact / within-tol at this PRE-FLIGHT cross-check). No new §7 trip.

### Inputs

- [x] `RECEIPTS/C8.18_step6a_2026-05-21T12-00-00Z.md` (the 10 forward-HALTs; the §7 finding/Option-A) — read.
- [x] `PRE_FLIGHT_LOG.md` 2026-05-21T00:00:00Z + the 12:00:00Z §7 addendum — read.
- [x] `~/Desktop/natality-harmonization/raw_docs/linked/LinkCO83Guide.pdf` (139pp; `producer="Acrobat PDFWriter 3.0"`, modDate 2007-11-30; embedded text layer per L12-extension) p4 (the 50%-non-VSCP-sample + record-weight methodology) + p13 0-idx (the coordinate-aligned Denominator "List of Data Elements and Locations": `1.f Record weight` = Denom byte **91**) + p12 0-idx (Denominator File: Record count 3,341,274; Record length 91; By occurrence 3,643,001; By residence 3,639,113) + `LinkCO84Guide.pdf` (sibling) — L9-read.
- [x] `~/Desktop/natality-harmonization/output/linked/linked_{1983,1984}_denomplus.parquet` (gitignored 6a `_raw`; carry RECWT@91 already) — read-only empirical re-derivation.
- [x] `natality/scripts/01_import/field_specs.py` `LINKED_BIRTH_1983_1988_FIELDS` (line 939 `("RECWT", 91, 91)`; `LINKED_DEN_RECLEN_1983_1988 = 91`) + `natality/scripts/03_harmonize/harmonize_linked_v3.py` `_harmonize_cohort_1983_1988` (lines 1168-1401; `record_weight` currently the dict-default NULL) + the 1995-2002 RECWT pattern (lines 874-878 `_to_float_or_null(recwt)`) — read.
- [x] 11 gate parquets — SHA byte-exact (above).

### Environment

- [x] `uv run python` / PyMuPDF (`fitz`) available; `pyarrow`/`pyarrow.compute` available. Working dir clean; branch `main`.

### Source documentation

- [x] LinkCO83Guide.pdf / LinkCO84Guide.pdf SHA-anchored at C8.18 DO step 2 (`docs/NCHS_SOURCE_MANIFEST.md` §3 / `natality/metadata/file_inventory.csv`); read-only here; text-layer non-empty (L12-extension probe ✓).

### Outputs (intended)

- `natality/scripts/03_harmonize/harmonize_linked_v3.py` — `_harmonize_cohort_1983_1988`: populate `record_weight` from RECWT@91 for year ∈ {1983,1984} (mirror the 1995-2002 `_to_float_or_null` pattern), era-gated so 1985-1988 stay NULL; docstring reword (the (A′) over-conservative-NULL → 6a-RECWT corrected; superseded for 1983-1984). **Additive/surgical; the 1983-1988 birth-side + den/num + 5c-i/5c-ii bodies byte-untouched.**
- `natality/scripts/01_import/field_specs.py` — **comment-only** clarification on the `("RECWT", 91, 91)` line (the 6a-RECWT empirical confirmation + the 1988 trailing-byte anomaly soft-flag (ll) note). **No position/behavior change** (the byte is correct → re-parse a no-op; `_raw` unaffected).
- `natality/tests/test_linked_cohort_6a_recwt_smoke.py` — NEW SMOKE harness (`DESIGN: tracks-current-state`; SHAPE-not-VALUE; §9-#9 RED→GREEN).
- `natality/tests/test_linked_cohort_5ciii_harmonize_smoke.py` — the minimal L17 `tracks-current-state` Edit (the 1983-1984 `record_weight==[None]` pins reframed; 1985-1988 stay NULL).
- `PRE_FLIGHT_LOG.md` (this) + `RECEIPTS/C8.18_step6a-RECWT_…` + `DECISION_LOG.md` 2026-05-22T02:00:00Z + `STATUS.md` — appended.
- **ZERO canonical parquet / `harmonized_schema.csv` / validation-CSV-row mutation** (the v3→v4 re-harmonize is 6b). 11 gate parquet SHAs stay byte-exact. No git tag (intermediate; `C8.18-pre-do`@`6632a15` rollback anchor).

### Field-value snapshot for cells / rows / columns being mutated (Convention 3)

| Artifact / row | Task-plan-assumed state (6a forward-HALT #4) | Actual verified state | Divergence resolution |
|---|---|---|---|
| `field_specs.py:939` `("RECWT", 91, 91)` | "the 3a @91 is WRONG per (A′); re-derive correct byte" | **@91 is CORRECT** — LinkCO83Guide.pdf p13 (coord-aligned) `1.f Record weight` = Denom byte 91; empirically Σ(RECWT@91 over full 1983 den, 3,341,274 rows) = **3,643,001 == guide "By occurrence" byte-exact**; values ∈ {"1":3,039,547; "2":301,727} per p4 methodology | NO position edit (assumed defect FALSIFIED); comment-only clarification. Within-task cheap-check scope refinement (5a/4a/4b precedent); not a §7 trip. |
| `harmonize_linked_v3.py` `_harmonize_cohort_1983_1988` `record_weight` | dict-default NULL for ALL 1983-1988 (5c-iii (A′) decision) | NULL for ALL 1983-1988 (confirmed current) | MUTATE: populate from RECWT@91 for year∈{1983,1984}; 1985-1988 stay NULL (§7 scope; SUPERSEDED-for-1983-1984 per DECISION_LOG 2026-05-21T12:00:00Z) |
| 1984 den Σ RECWT vs guide "By occurrence" 3,673,693 | (n/a) | Σ = 3,673,694 (off-by-1; late-filed/rounding; within any tol) | accepted (1-record; H6 record-count gate 3,364,090 byte-exact) |
| `external_validation_targets_v3_linked.csv` `resident_births,1983/1984` = 3,639,113 / 3,669,268 (state-on-disk; do NOT re-source) | weighted target | Σ(RECWT@91 \| link_segment="den" ∧ RESSTAT≠4): **1983 = 3,639,113 byte-exact; 1984 = 3,669,268 byte-exact** | VERIFY criterion pre-confirmed |
| `…csv` `resident_infant_deaths,1983/1984` = 39,683 / 38,294 (±2); `imr_per_1000` = 10.9 / 10.44 (±0.02) | num/IMR target | num\|RESSTAT≠4 (uniform RECWT="1"): 1983 = 39,683 (exact), 1984 = 38,292 (Δ2, in tol); weighted IMR 1983 = **10.905** (in tol of 10.9), 1984 = **10.436** (in tol of 10.44) | VERIFY criterion pre-confirmed |
| 1985-1988 den RECWT@91 (soft-flag (ll) spot) | (n/a) | 1985 "1"×3,765,336 / 1986 "1"×3,760,997 / 1987 "1"×3,813,491(+1 stray) — full files, uniform weight 1 → NULL correct; **1988 multi-valued, byte91==byte57 ~100%** — genuine trailing-byte anomaly, but full file (RC==occ) needs NO weighting → NULL correct + harmless; per-year 1985-1988 layout reconstruction = 3a-class future task (soft-flag (ll) carry, NOT 1983-2023-IMR-blocking) | resolves (A′); 1983-1984-only scope confirmed |

### Halt conditions tripped

None. The task-plan-assumed defect (3a RECWT@91 wrong) is **FALSIFIED** at the cheap-check (the §2 cheap-before-expensive PRE-FLIGHT working as designed — the 5a/4a/4b "prior assumption FALSIFIED → smaller scope" precedent). The §7/§8 finding + Option-A disposition (1983-1984 IMR biased without weighting) **stand and are executed**; the corrected weighted figures **reconcile byte-exact / within-tol** (no new §7 #4/#13 trip — forward-HALT #10's "re-halt only if still divergent" does not fire). Scope refined SMALLER (harmonize-only; no field_specs position edit; no re-parse) — §7-#17 scope-creep does NOT apply (scope shrank within the declared 6a-RECWT boundary; no files outside scope touched).

### Result

**PROCEED** to SMOKE (§9-#9 SMOKE-first) → DO (harmonize-only + L17 bundle + field_specs comment) → VERIFY → RECEIPT. L10-safe: this PRE-FLIGHT precedes the first DO mutation.

---

## PRE-FLIGHT ADDENDUM for C8.18 DO step 6a — 2026-05-21T12:00:00Z — a binding **§7 #4/#13 + §8 halt-and-ask** surfaced at the DO-step-6a full-file parse: the **1983-1984 cohort-linked denominator is a documented 50%-of-births-in-5-non-VSCP-areas weighted sample**, so the harmonized 1983-1984 IMR (`record_weight` NULL'd at 5c-iii) is ~8-9% biased vs published NCHS cohort figures — a **latent defect in already-completed 5c-iii work**; surfaced via `AskUserQuestion` (airtight finding + 3 concrete options + the defect-in-completed-work explicitly disclosed); the user delegated judgment ("do whatever you think is the best") → **Option A (fix the root cause)**; the §7 scope **refined 1983-1988 → 1983-1984 ONLY** by the L9-sourced guide control totals; DO step 6 **re-decomposed → 6a / 6a-RECWT / 6b** (a NEW dated entry, NOT a back-fill — the 5c-iii 2026-05-20T02:00:00Z §7-addendum precedent; L10-safe — written BEFORE the DO step 6a commit)

> The §7/§8 halt-and-ask was performed + satisfied (the binding KICKOFF/§7/§9-#3/anti-pattern-#7 discipline; I stated at kickoff I would halt-and-ask on any §7 gate regardless of the standing authorization, HALT 7 especially — and HALT 7 fired exactly as the 5c-iii receipt designed: "per-year cohort IMR diverges from NCHS → §7 halt"). The user responded "do whatever you think is the best" (delegated judgment, the 5c-iii precedent: surfaced + options + defect disclosed → judgment exercised). Judgment = **Option A**. This addendum records the finding + the decision + the re-decomposition before the DO step 6a commit (L10-safe; zero canonical parquet/schema-CSV/`field_specs.py` mutation this sub-step — the RECWT root-cause fix is the separate 6a-RECWT sub-step with its OWN PRE-FLIGHT).

### The §7 finding (airtight; L9-sourced from the SHA-anchored cohort guide PDFs)

The DO-step-6a full-file parse + the L9 reading of all 19 cohort guide "Machine/File/Data Characteristics" sections established:

- **1983, 1984** ONLY: the cohort-linked DENOMINATOR public-use file is a **documented weighted sample** — `LinkCO83Guide.pdf` p4: in 1983 DC + Arizona + California + Delaware + Georgia were non-VSCP, only **50% (even-numbered) of their birth certificates were coded** into the natality/denominator file; RECWT=2 inflates them. Guide den **Record count ≠ by-occurrence**: 1983 = 3,341,274 vs 3,643,001 (Δ8.3%); 1984 = 3,364,090 vs 3,673,693 (Δ8.4%). The harmonized 1983/1984 cohort IMR as designed at 5c-iii (`record_weight`=NULL → unweighted `count(num)/count(den)`) = 39,704/3,341,274×1000 = **11.88** vs the NCHS-published weighted ≈ 39,683/3,639,113×1000 ≈ **10.90** — a **~9% upward bias matching no published NCHS figure**.
- **1985-2004** (15 years): den **Record count == by-occurrence** (full files; verified all years) → unweighted == published-comparable (modulo the standard occurrence→residence reallocation handled by the canonical `residence_status != 4` filter). The §7 weighting issue does **NOT** apply here. The 5c-iii `record_weight`=NULL framing ("the cohort IMR = count(num)/count(den) does not use record_weight") is **correct for 1985-2004** and **a latent defect for 1983-1984 only**.
- This trips **§7 #4** (per-year rate disagreement ~9%, 1983-1984), **§7 #13** (validity-domain: the unweighted 1983-1984 IMR corresponds to no published statistic), **§8** (a defect in already-completed 5c-iii work — the `record_weight`=NULL decision + the receipt framing; compounded by the 5c-iii (A′) finding that the 3a-transcribed `RECWT@91` byte is itself wrong = "1" for 1983/85, = DMRACE for 1988). The 5c-iii `record_weight`=NULL decision is **superseded for 1983-1984** by `DECISION_LOG.md` 2026-05-21 (§3 append-only-supersede / §11 backport).

### The §7 disposition — Option A (fix the root cause), by delegated judgment

Per anti-pattern #7 (never ship a knowingly ~9%-biased product behind a caveat when the root cause is fixable) + the §15.D VERIFY criterion itself ("per-year cohort counts 1983-2004 match NCHS cohort Linked File report tables byte-exact" — only Option A achieves it) + the project's "maximally robust" mandate + the fact that RECWT is a **documented recoverable field** (the 1983 guide data-elements list element "1.f Record weight"; the soft-flag (ll) RECWT-location re-derivation was already scheduled — Option A elevates it from optional to required) + the §7 scope being a contained **2 years**. **Option B** (ship unweighted + caveat) and **Option C** (drop 1983-1984/1983-1988) were rejected (B ships a known-biased product against the byte-exact-validation discipline; C abandons recoverable coverage against the mandate).

### Re-decomposition: DO step 6 → 6a / 6a-RECWT / 6b (the 5→5a/5b/5c precedent; soft-flag (ii) §11 human-merge)

- **DO step 6a (THIS sub-step, now closing)**: the additive `parse_all_linked_years._zip_name_for_year` pre-2005 mapping extension (SMOKE 17/17 incl. the §7-fixed CHUNKED `run_parse` at the den→num boundary on real `LinkCO83`/`LinkCO88` — the gap the 5c-iii self-check #2 flagged, now closed) + the full-file parse of the 19 pre-2005 cohort zips → `output/linked/linked_{year}_denomplus.parquet` (gitignored intermediates) + the L9 sourcing of all 19 years' den/num control totals into `external_validation_targets_v3_linked.csv` (additive; existing 2005-2023 rows byte-exact preserved) + the **H6 byte-exact parser-conservation VERIFY** (parsed `_raw` den/num == guide File "Record count", byte-exact — PASS for all parsed years) + the §7 finding + Option-A decision. **ZERO canonical parquet / `harmonized_schema.csv` / `field_specs.py` mutation.**
- **DO step 6a-RECWT (NEXT; its OWN PRE-FLIGHT — it mutates `field_specs.py` canonical substrate + reopens a 5c-iii decision)**: re-derive the correct 1983-1984 RECWT byte position from the LinkCO83/84 guide record-layout sections (3a-class; + the soft-flag (ll) full per-year 1983-1984 non-anchor re-verification), edit `field_specs.py`, reopen `_harmonize_cohort_1983_1988` `record_weight` (correctly-located RECWT, weighted counts for 1983-1984; 1985-1988 stay effectively weight-1), re-parse 1983-1984, VERIFY the WEIGHTED den/IMR == guide by-residence / NCHS-published (tolerance) — §7 halt if still divergent.
- **DO step 6b**: re-harmonize linked 1983-2023 → v4 (preserve v3 `.v3_baseline`) + re-derive + `harmonized_schema.csv` HALT 6 (a)-(f) + v3→v4 bump + Convention-2/L17 re-pins + the full §15.D VERIFY (per-year cohort counts 1983-2004 byte-exact incl. the weighted 1983-1984; 2005-2023 byte-clean regression; 1992-1994 gap docs) + the D.4 within-era/coverage framing.

### Result
**DO step 6a CLOSING** (parse-substrate + the L9 control-total sourcing + the H6 byte-exact VERIFY + the §7 finding/decision/re-decomposition). The §7/§8 halt-and-ask satisfied. **DO step 6a-RECWT** (the bounded 1983-1984 root-cause fix) is the next sub-step with its own PRE-FLIGHT; then **6b**. No tag (intermediate; `C8.18-pre-do`@`6632a15` rollback anchor). soft-flag (ii) §11 human-merge owes the human the 6→6a/6a-RECWT/6b re-decomposition + the §7-finding model note.

---

## PRE-FLIGHT for C8.18 DO step 6 — 2026-05-21T00:00:00Z — **re-harmonize linked 1983-2023 → v4** (the FIRST canonical-state mutation in C8.18) — **DECOMPOSED → DO step 6a / 6b** at this Convention-3 snapshot (the 5→5a/5b/5c + 5c→5c-i/5c-ii/5c-iii + 4→4a/4b/4c + 3→3a/3b established precedent + the user's standing 2026-05-21 "proceed and make all relevant decisions by yourself in the best way" authorization) — **RESULT: PROCEED to DO step 6a** (10/10 forward-looking HALTs from `RECEIPTS/C8.18_step5c-iii_2026-05-20T02-00-00Z.md` re-verified; **11/11 gate parquet SHAs byte-exact**; canonical pytest baseline `303 passed, 1 skipped, 1 xfailed in 401.62s` (4-dir suite) — the COUNT is the 5c-iii-close baseline (the gate); 401.62s within the ~340-680s advisory band → soft-flag (kk) carry; two Convention-3 findings surfaced + resolved at this cheap-check **before any DO mutation**: (G) `external_validation_targets_v3_linked.csv` carries NO pre-2005 cohort targets → DO step 6a must source them from the 19 on-disk SHA-anchored cohort guide PDFs (the §15.D "new pre-2005 cohort cells added incrementally" deliverable; not new scope), (H) the existing `parse_all_linked_years.py` driver hard-codes the `LinkCO{yy}US.zip` 1995+ naming → DO step 6a must additively extend the pre-2005 year→zip mapping (1983-1991 = `LinkCO{yy}.zip` no-US; 1995-2004 = `LinkCO{yy}US.zip`); zero §7 halts at PRE-FLIGHT — the §7 latent-defect halt-and-ask was satisfied + closed at 5c-iii; HALT 7 is a DO-step-6a VERIFY-gate, not a PRE-FLIGHT trip)

> Per-sub-step PRE-FLIGHT under the C8.18 umbrella PRE-FLIGHT (2026-05-17T05:30:00Z) + the §15.D DO step 5 → 5a/5b/5c (2026-05-19T05:00:00Z) + 5c → 5c-i/5c-ii/5c-iii (2026-05-19T11:00:00Z) decompositions + the NEW DO step 6 → **6a/6b** decomposition fixed at this snapshot. Written **before any DO step 6 mutation** (no parser-driver edit, no parse, no schema-CSV edit, no re-harmonize yet — the only writes from this entry are PRE_FLIGHT_LOG + state files; L10-safe). DO step 6 is the FIRST C8.18 canonical-state mutation (3a-5c-iii were all zero-canonical-mutation substrate); the decomposition isolates the zero-canonical-mutation parse + validation-target-sourcing + the critical HALT 7 §7 end-to-end gate (= DO step 6a) BEFORE the canonical v3→v4 re-harmonize + schema-CSV edit + version bump (= DO step 6b), per §2 cheap-before-expensive.

### Entry cheap-check — 10 forward-looking HALTs (from `RECEIPTS/C8.18_step5c-iii_2026-05-20T02-00-00Z.md`)

1. **PASS** — `C8.18-pre-do`@`6632a15` (the C8.18 DO rollback anchor); `C8.18-complete` NOT present (final-sub-step-only, set at DO step 7); `C8.17-complete` present; HEAD = `4cb5a2c` (the DO step 5c-iii commit, after `f940b46`=5c-ii-b). Verified `git rev-parse`/`git tag`/`git log`; working tree clean on `main` (the start-of-session "uncommitted" snapshot was stale — 5c-iii is committed at `4cb5a2c` with all 11 expected files).
2. **PASS** — **11/11 gate parquet SHAs byte-exact** vs the 5c-iii HALT 2 set (`/tmp/c8_18_s6/` recompute; 16-hex prefixes): nat `c8a740eb…`(harm)/`acb5c48a…`(deriv); `.v28_baseline` `230efed2…`/`e16ad532…`; linked-deriv `9b828a4d…`; fetal-death `38e2cecb…`(harm)/`185c071e…`(deriv) (via the `~/Desktop/fetal-death-harmonization-build/` tree, soft-flag (hh)); MM `5c22308b…`(1995-1997)/`7c682668…`(1995-2000)/`d98b4296…`(2016-2020)/`adbec108…`(harm). The linked-derived `9b828a4d…` changes ONLY at the DO step 6b re-harmonize (6a is parse-intermediate + validation-target-sourcing only — zero gate-parquet/schema-CSV mutation).
3. **PASS** — canonical **4-dir** suite `fetal_death/tests/ natality/tests/ tests/ matched_multiples/tests/` = `303 passed, 1 skipped, 1 xfailed in 401.62s` (`/tmp/c8_18_s6/pytest_baseline.out`, exit 0); the COUNT is exactly the 5c-iii-close baseline (303P+1S+1XF) — the gate per soft-flag (kk)/§8 L17/HALT-3. 401.62s within the ~340-680s advisory band. README 3-dir/"56 passed" line stays stale → soft-flag (jj).
4. **PASS** — `harmonize_linked_v3` exposes `_harmonize_cohort_1983_1988` + `_mrace1digit_to_bridged4` + `_harmonize_cohort_2003_2004` + `_harmonize_cohort_1995_2002` + `_cohort_era` + `_harmonize_cohort_batch` + `_harmonize_batch` (the `if _cohort_era(year) is not None: return _harmonize_cohort_batch(...)` dispatch at line 1644; year≥2005 → the byte-untouched 2005+ body) + `OUT_SCHEMA` **81 cols** (`['…','cause_recode_61','manner_of_death','record_weight','link_segment']`; `underlying_cause_icd9`+`cause_recode_61`+`link_segment` present — the 5c-i/5c-iii additive code-schema extension). `parse_linked_year` exposes `_expected_parsed_schema` (1983-1988 → den∪num union; non-1983-1988 → None) + the §7-fixed `run_parse` (chunk_rows default 250_000; the explicit `_schema` wiring). `parse_all_linked_years.py` (the 2005-2015 driver) + `parse_linked_year.run_parse` state-on-disk; verified via `uv run python -c "import …"`. **State-on-disk; do NOT re-derive / do NOT re-open** the 5b construction, the within_era ICD-9/10 shape, or the §7 fix.
5. **PASS / noted** — the within_era ICD-9/ICD-10 cause-column-shape is **RESOLVED + state-on-disk** (§15.D DO step 1; 5c-i/ii-a/ii-b/iii applied it). 1983-1988 = **ICD-9** (`underlying_cause_icd9`=UCOD / `cause_recode_61`=UCODR61 on num rows; ICD-10 cols null; **NO `age_at_death_days`** — only `age_at_death_recode5`=AGER5). 1995-1998 = ICD-9; 1999-2004 = ICD-10. Do NOT re-open.
6. **PASS / enumerated for DO step 6b** — the `harmonized_schema.csv` metadata edit + the v3→v4 version bump = **DO step 6b** (Anti-Pattern #6). The Convention-3 Field-value snapshot below enumerates HALT 6 (a)-(f) against the **actual current** `natality/metadata/harmonized_schema.csv` state (rows 86-95 = the linked death-side; `link_segment`/`underlying_cause_icd9`/`cause_recode_61` rows ABSENT — to be added; the schema CSV's top-of-file has NO explicit `version:` field — the version is carried in product docs/CITATION/.zenodo (DO step 7) + the row-5 `certificate_revision` DECISION_LOG-referenced comment-style precedent). DO step 6b PRE-FLIGHT (its own per-sub-step entry) will re-snapshot before the edit.
7. **PASS / planned into DO step 6a** — **the NEW critical DO-step-6 §7 gate**: the §7-fixed `run_parse` is state-on-disk but DO step 6a MUST value-distribution-verify the actual full-file 1983-1988 `_raw` parquet preserves the FULL numerator ICD-9 section + the den/num per-year H6 row counts (den == NCHS-stated births; num == NCHS-stated infant deaths) + the per-year cohort IMR vs the NCHS published cohort Linked File report. **This is a DO-step-6a VERIFY-gate, not a PRE-FLIGHT trip** (the parse has not run; the §7 latent-defect halt-and-ask was satisfied + closed at 5c-iii with the human-authorized Option-A root-cause fix). If at DO step 6a the 1983-1988 num section is empty / den≈num / per-year cohort IMR diverges from NCHS → **§7 halt-and-ask** (the durable end-to-end check that the §7 fix + the RESOLVED-5b construction are correct).
8. **PASS / consumed** — soft-flag (gg) carries: the cause-column SHAPE is resolved (HALT 5); the composite-block leaf decomposition (DELMTH/MEDRISK/OBSTETRC/LABOR/ABNORMNB/ENTITY/RECORDAX + smoking/diabetes/hypertension/preterm_recode3 + the 2-digit ORMOTH/ORFATH Hispanic crosswalk) stays conservatively NULL → DO step 6/post-C8.18. **soft-flag (ll)** (the RECWT@91 byte-91 1986-1988 layout-instability + the FULL per-year 1984-1988 non-anchor byte-stability re-verification) is routed to **DO step 6a** (the parse + the NCHS-per-year-cohort-count cross-check is the durable §15.D root-cause gate the 3a self-check #2 + 5b self-check #3 scheduled here; `field_specs.py` UNTOUCHED — a per-year layout reconstruction, if 6a surfaces analytic-validity-affecting non-anchor drift, is a §7 halt + a 3a-class task).
9. **PASS / noted** — §15.D substrate-format reconcile applied at the 3a commit; the broader model-clarification (3a/3b + 4a/4b/4c + 5→5a/5b/5c + 5c→5c-i/5c-ii/5c-iii + 5c-ii→5c-ii-a/5c-ii-b + the 5c-iii keyless map + the (A′) RECWT@91 conservative-NULL + the §7 `run_parse` fix + **NOW the DO step 6 → 6a/6b decomposition**) remains **soft-flag (ii) §11 human-merge** (proposed-not-applied; on-disk PRE_FLIGHT_LOG/DECISION_LOG/RECEIPTs/LESSONS authoritative meanwhile). The LESSONS 2026-05-20 §8-row-sharpening (L13-extension year-axis + H6 heterogeneous-union) is owed §11 human-merge.
10. **PASS / action queued for DO step 6b** — L17 grep-scope of `natality/tests/` + `tests/` (`git ls-files | xargs grep`): NO `NotImplementedError` keyless-1983-1988 pin remains (the 3 reframed at 5c-iii). The cohort harmonize harnesses assert `schema == OUT_SCHEMA` *relatively* (no hardcoded col-count) → adding to OUT_SCHEMA keeps them green. The **DO-step-6b** Convention-2/L17 same-commit re-pin targets are identified: `tests/test_row_count_conservation.py` `LINKED_EXPECTED_TOTAL = 74_943_824` (line 48; → the v4 1983-2023 total) + `LINKED_EXPECTED_YEARS = list(range(2005, 2024))  # 19 contiguous` (line 49; → 1983-1991 ∪ 1995-2023 = 38 years WITH the permanent 1992-1994 gap, NOT contiguous) + `test_linked_harmonized_total_matches_release_state` / `test_linked_year_coverage_matches_expected` (Convention-2 release-state pins) + the B.12 `tests/test_parquet_column_snapshot.py` re-snap (a new `tests/snapshots/v3_<UTC>_columns.csv` for the v4 linked parquet) + `natality/tests/test_schema_dtype_parity.py` must PASS on the v4 parquet (the new schema rows ↔ v4 dtypes). 6a configures no new harness pin (parse-intermediate + additive validation-CSV only — the SMOKE harness for 6a is `DESIGN: tracks-current-state`, SHAPE-not-VALUE, sub-step-isolated).

### Inputs
- [x] `natality/scripts/01_import/parse_linked_year.py` — `run_parse` (the §7-fixed; `_expected_parsed_schema(year)`; chunk_rows default 250_000) + `iter_parsed_records` (1983-1988 → `_iter_two_file_1983_1988`; else `_layout_for_linked_year` + `_find_denomplus_member`) — read in full; the §7 fix RESOLVED + state-on-disk (do NOT re-open).
- [x] `natality/scripts/01_import/parse_all_linked_years.py` — the 2005-2015 batch driver (`zip = linked-dir/f"LinkCO{yy}US.zip"`; `out = out-dir/f"linked_{year}_denomplus.parquet"`; default `--years 2005-2015`). **(H) finding**: hard-codes the `LinkCO{yy}US.zip` 1995+ naming → DO step 6a must additively extend the pre-2005 mapping (1983-1991 = `LinkCO{yy}.zip` no-US suffix; 1995-2004 = `LinkCO{yy}US.zip`). 2016-2023 use `parse_linked_cohort_year.py` (period-cohort `{YYYY}PE{YYYY-1}CO.zip`) — out of 6a scope (the 2005-2023 _raw parquets already exist + are byte-clean-regression-anchored; 6a does NOT re-parse them).
- [x] `natality/scripts/03_harmonize/harmonize_linked_v3.py` — `main()` reads `args.linked_dir/f"linked_{year}_denomplus.parquet"` per year, `_harmonize_batch(batch,year)` (auto-routes `_cohort_era(year) is not None` → `_harmonize_cohort_batch`), writes `OUT_SCHEMA` (81 cols) to `args.out` (default `output/harmonized/natality_v3_linked_harmonized.parquet`); `--years` default `2005-2023`. State-on-disk; the re-harmonize is DO step 6b.
- [x] `natality/scripts/04_derive/derive_linked_v3.py` — the linked derive stage (DO step 6b re-derive after re-harmonize).
- [x] 19 pre-2005 cohort source zips present + SHA-anchored (C8.18 DO step 2) at `~/Desktop/natality-harmonization/raw_data/linked/`: `LinkCO{83..91}.zip` (9) + `LinkCO{95..04}US.zip` (10). The 2005-2023 zips + the existing `output/linked/linked_{2005..2023}_denomplus.parquet` _raw intermediates present (byte-clean-regression anchor — 6a does NOT touch them).
- [x] 19 pre-2005 cohort guide PDFs present + SHA-anchored (C8.18 DO step 2) at `~/Desktop/natality-harmonization/raw_docs/linked/`: `LinkCO{83..91,95..99,00..04}Guide.pdf` (the DO step 6a validation-target source; sibling-derived from the on-disk `LinkCO05/10/15Guide.pdf` Documentation-Table structure; L9/L12-extension cheap-check at 6a).
- [x] `natality/metadata/harmonized_schema.csv` — read in full (96 rows; rows 86-95 = linked death-side; the Convention-3 snapshot below). DO step 6b edit (HALT 6).
- [x] `natality/metadata/external_validation_targets_v3_linked.csv` — read in full. **(G) finding**: targets ONLY for 2005/2010/2015/2020/2021/2022/2023 (the existing 33/35 + 2-docs grid). NO 1983-2004 rows → DO step 6a sources + additively appends pre-2005 cohort cells (existing rows preserved byte-exact).
- [x] `natality/metadata/file_inventory.csv` rows 55-73 — the 19 cohort zips SHA-anchored at DO step 2 (`imported=false` until the C8.18 re-harmonize; per-member byte sizes recorded — den/num/unl). DO step 6b flips `imported=true`.
- [x] `tests/test_row_count_conservation.py` (LINKED pins lines 48-49 + the release-state tests) + `tests/test_parquet_column_snapshot.py` + `tests/snapshots/v2_2026-05-16T08-00-00Z_columns.csv` + `natality/tests/test_schema_dtype_parity.py` — the DO-step-6b Convention-2/L17 re-pin + VERIFY targets (HALT 10).
- [x] `RECEIPTS/C8.18_step5c-iii_2026-05-20T02-00-00Z.md` (the 10 forward-looking HALTs) + `RECEIPTS/C8.18_step5b_…` (the RESOLVED keyless two-file construction model) + `RECEIPTS/C8.18_step3a_…` (self-check #2 — the per-year non-anchor verification routed to DO step 5/6).
- [x] No required upstream task incomplete: DO step 1-5c-iii CLOSED + committed (HEAD `4cb5a2c`).
- [x] No stale checkpoints: `git status --porcelain` clean; on `main`; `/tmp/c8_18_s6/*` is OS-scratch (gate-SHA + pytest baseline), reproducible.

### Environment
- [x] Python 3.13 via `uv run`; pyarrow/pandas pinned (`uv.lock`, C8.5a). No new dependency at DO step 6 (parser + harmonizer + derive substrate already in place; 6a adds only a parse-driver year→zip mapping extension + a validation-target-sourcing read of on-disk PDFs via PyMuPDF, already a dependency from the C8.17/C8.18 doc-probe precedent).
- [x] Working dir clean; on `main`.

### Source documentation
- [x] The 19 pre-2005 cohort guide PDFs are SHA-anchored at C8.18 DO step 2 (`file_inventory.csv` `doc_file` column). DO step 6a re-verifies the named Documentation Table location in each actual PDF before sourcing the per-year control totals (L9 cheap-check) + a `page.get_text()` text-layer probe (L12-extension — the 1980s NCHS PDFs may be bitmap+text-layer like the fetal 1985 guide; the cohort guides 2005/2010/2015 already text-extract). Any guide whose Documentation Table is bitmap-only without a text layer → L12-extension probe logged + OCR-or-manual-transcribe decision surfaced at 6a (not fabricated; sibling-derived).

### Outputs (intended)
**DO step 6a (this session, after SMOKE):**
- [ ] `natality/scripts/01_import/parse_all_linked_years.py` (or a minimal sibling driver) — **additive** pre-2005 year→zip mapping (1983-1991 → `LinkCO{yy}.zip`; 1995-2004 → `LinkCO{yy}US.zip`); the 2005-2015 `LinkCO{yy}US.zip` path byte-untouched (§9-#7-safe).
- [ ] `output/linked/linked_{1983..1991,1995..2004}_denomplus.parquet` — NEW gitignored _raw intermediates (19; the §7-fixed chunked `run_parse`; 1983-1988 keyless den∪num union; 1989-1991 + 1995-2004 denominator-plus). The 2005-2023 _raw byte-untouched.
- [ ] `natality/metadata/external_validation_targets_v3_linked.csv` — **additive** pre-2005 cohort target rows (resident_births / unweighted_infant_deaths / imr_per_1000 per year 1983-2004, sourced from the 19 guide PDFs; the existing 2005-2023 rows byte-exact preserved). Metadata CSV, NOT version-gated (Anti-Pattern #6 applies only to `harmonized_schema.csv`).
- [ ] NEW `natality/tests/test_linked_do6a_parse_smoke.py` (or extend an existing cohort harness) — `DESIGN: tracks-current-state`; SHAPE-not-VALUE; the §7 end-to-end + the parse-driver-extension SMOKE. Written FIRST + run RED (§9-#9).
- [ ] `PRE_FLIGHT_LOG.md` (this entry) + `RECEIPTS/C8.18_step6a_…` + `DECISION_LOG.md` + `STATUS.md` appended; LESSONS only if a new class.
- [x] **6a: ZERO canonical parquet / `harmonized_schema.csv` / `field_specs.py` mutation** (parse-intermediates are gitignored + reproducible; the validation-target CSV addition is additive metadata; the 11 gate parquet SHAs byte-exact). NO git tag (intermediate DO step; `C8.18-pre-do`@`6632a15` is the rollback anchor).

**DO step 6b (next session — its own per-sub-step PRE-FLIGHT):** re-harmonize linked 1983-2023 → v4 (preserve v3 as `.v3_baseline.parquet`); re-derive; `harmonized_schema.csv` HALT 6 (a)-(f) edit + v3→v4 bump (Anti-Pattern #6); Convention-2/L17 same-commit re-pins (`test_row_count_conservation.py` LINKED total/year-list + the release-state tests + B.12 re-snap); VERIFY (per-year cohort counts 1983-2004 vs the 6a-sourced NCHS targets byte-exact; 2005-2023 byte-clean regression vs `.v3_baseline` — 0 drift on the 80 v3 cols, `link_segment` all-NULL there; 1992-1994 gap documented; C8.1 dtype-parity PASS on v4; external validation 33/35 + 2 docs preserved + the new pre-2005 cells); the canonical-state mutation.

### Field-value snapshot for cells / rows / columns being mutated (Convention 3)

DO step 6a mutates NO canonical *value* (parse-intermediates + additive validation-CSV). DO step 6b mutates `harmonized_schema.csv` (HALT 6) + the linked parquet (v3→v4). Snapshot of the **current** state (read 2026-05-21, before any DO mutation), grounded in the actual on-disk files:

**(Snapshot S1) `natality/metadata/harmonized_schema.csv` — the linked death-side rows + the absent rows (HALT 6 (a)-(f) targets for DO step 6b):**
- Rows 86-95 (`infant_death`, `age_at_death_days`, `age_at_death_recode5`, `underlying_cause_icd10`, `cause_recode_130`, `manner_of_death`, `record_weight`, `neonatal_death`, `postneonatal_death`, `cause_group`) ALL carry `years_available = "2005-2023 (linked)"` and `notes` containing "V3 linked files only" — **HALT 6(a)**: widen to 1983-2023 with the 1992-1994 gap.
- `manner_of_death` (row 91) `years_available = 2005-2023 (linked)` — **HALT 6(c)**: cohort 2003-2004 only on the pre-2005 side (5c-ii-b maps MANNER); null 1983-2002 incl. 1983-1988 (no MANNER in the 1983-1988/1995-2002 numerator).
- `father_age_cat_from_rec11` (row 39) `years_available = 2005-2013`, `comparability_class = within-era` — **HALT 6(d)**: the cohort side (5c-ii-b) enriches 2003-2004; widen accordingly.
- **ABSENT rows (to ADD at 6b, HALT 6(b)+(e))**: NO `underlying_cause_icd9` row, NO `cause_recode_61` row, NO `link_segment` row. (All three ARE in code `OUT_SCHEMA` since 5c-i/5c-iii — the metadata-CSV rows + the v3→v4 bump are the DO step 6b deliverable per Anti-Pattern #6.) `link_segment` row must carry the within-era structural-difference note (1983-1988 IMR = count(num)/count(den), NOT a per-birth `infant_death` filter — the D.4 Coverage item) + the 1983-1988 `record_weight`/`hispanic*` null-pattern + the "no `age_at_death_days`, AGER5-only" note + cert_rev "unrevised_1968".
- The schema CSV has no top-of-file `version:` token; the product version (v3→v4) lives in CITATION.cff/.zenodo.json/ABOUT_THIS_RELEASE/README (= DO step 7) + the Anti-Pattern-#6 DECISION_LOG-referenced edit-comment precedent (row 5 `certificate_revision` notes "version bumped v2.8.0->v3.0.0 … See DECISION_LOG …"). DO step 6b adds the analogous v3→v4 DECISION_LOG-referenced provenance.

**(Snapshot S2, finding (G)) `natality/metadata/external_validation_targets_v3_linked.csv`:** 33 active target rows + 4 commented (the 2021 neo/postneo EXCLUDED investigation) — ALL for years ∈ {2005,2010,2015,2020,2021,2022,2023}. ZERO rows for 1983-2004. The §15.D VERIFY ("per-year cohort counts 1983-2004 match NCHS cohort Linked File report tables byte-exact" + "new pre-2005 cohort cells added incrementally") + HALT 7 require these — DO step 6a sources them from the 19 SHA-anchored guide PDFs. **Resolution:** in-scope for C8.18/DO-step-6 (the §15.D VERIFY mechanism explicitly anticipates "new pre-2005 cohort cells added incrementally"); NOT a Q42/Phase-B-2 trigger (the planned VERIFY-enabling work, not a new candidate). The supported metrics + the cohort "do not apply the weight" convention are documented in the CSV's trailing notes (the 2015+ unweighted-cohort precedent governs the pre-2005 sourcing).

**(Snapshot S3, finding (H)) `parse_all_linked_years.py`:** lines 52-53 hard-code `yy=f"{year%100:02d}"; zip=f"LinkCO{yy}US.zip"`. Correct for 1995-2015; WRONG for 1983-1991 (`LinkCO{yy}.zip`, no `US`). DO step 6a additively extends the mapping (the 1995-2015 path byte-untouched; the 2016-2023 period-cohort path is `parse_linked_cohort_year.py`, out of 6a scope — 6a does NOT re-parse the byte-clean-regression-anchored 2005-2023 _raw).

**(Decision D-6/6a-6b) DO step 6 → 6a/6b decomposition.** Per §2 cheap-before-expensive + the 5→5a/5b/5c + 5c→5c-i/5c-ii/5c-iii + 4→4a/4b/4c + 3→3a/3b established within-task-decomposition precedent + the user's standing 2026-05-21 authorization: **DO step 6a** = the parse-driver pre-2005 mapping extension + the full-file parse of the 19 pre-2005 cohort zips → `linked_{year}_denomplus.parquet` _raw intermediates + the pre-2005 NCHS cohort validation-target sourcing from the 19 guide PDFs + the **HALT 7 §7 end-to-end gate** (the 1983-1988 numerator-ICD-9-section-preserved + den/num per-year H6 counts + per-year cohort IMR vs NCHS) + **soft-flag (ll)** the full per-year 1984-1988 non-anchor byte-stability re-verification — **ZERO canonical parquet/schema-CSV/field_specs mutation** (the §15.D-VERIFY-enabling, highest-§7-risk substrate work isolated FIRST). **DO step 6b** = the canonical v3→v4 re-harmonize 1983-2023 + re-derive + the `harmonized_schema.csv` HALT 6 (a)-(f) edit + the v3→v4 bump (Anti-Pattern #6) + the Convention-2/L17 same-commit re-pins + the §15.D VERIFY + the 1992-1994-gap/within-era-structural-difference doc updates — the canonical mutation. Folds into soft-flag (ii) §11 human-merge (proposed-not-applied; the 5→5a/5b/5c precedent). NOT scope creep (same C8.18 §15.D DO-step-6 scope, decomposed for cheap-before-expensive sequencing); NOT a Q42/Phase-B-2 trigger (decomposition, not a new >1-session candidate — cumulative Phase C ≈ 35 of 51-71, cap 86 intact).

- [x] Current values match the (decomposed) task plan's assumed state ✓ — divergences (G)/(H) + the (D-6) decomposition named + resolved at this cheap-check moment per the established precedent + the standing authorization; no silent proceed under a divergent state. (G)/(H) are in-scope DO-step-6a work (the §15.D-prescribed VERIFY-enabling sourcing + the additive parse-driver extension), NOT new scope, NOT a §7 condition. HALT 7 is a DO-step-6a VERIFY-gate (the parse has not run) — evaluated against §7 below.

### Halt conditions tripped
None at PRE-FLIGHT. (G)/(H)/(D-6) are real-data-grounded in-scope resolutions at the Convention-3 snapshot per the 5→5a/5b/5c precedent + the standing authorization; not new scope, not a Q42/Phase-B-2 trigger, not a §7 condition. **HALT 7 explicitly evaluated against §7**: it is a DO-step-6a VERIFY criterion (NOT yet tripped — the parse has not run); the §7 latent-defect halt-and-ask it descends from was surfaced + halt-and-asked + human-authorized + RESOLVED (Option A) at 5c-iii (closed, not open). IF DO step 6a's full-file parse shows the 1983-1988 num ICD-9 section empty / den≈num / per-year cohort IMR diverging from the sourced NCHS targets → that is a §7 #3/#4 trip AT DO STEP 6a, surfaced via halt-and-ask THEN (the durable end-to-end gate the 5c-iii receipt self-check #2/#6 + HALT 7 scheduled). soft-flag (ii) §11 human-merge owes the human the model-clarification (now also folding the DO step 6 → 6a/6b decomposition; proposed-not-applied). soft-flag (ll) (RECWT@91 + the full per-year 1984-1988 non-anchor re-verification) is consumed at DO step 6a (its scheduled place). soft-flag (kk): pytest 401.62s within the advisory band (the COUNT 303P+1S+1XF is the gate). soft-flag (jj): README 3-dir/"56 passed" stale.

### Result
**PROCEED to DO step 6a** (SMOKE harness authored FIRST + run RED per §9-#9 [the parse-driver pre-2005 extension + the §7 end-to-end on real `LinkCO83`/`LinkCO88`]; then the additive parse-driver mapping extension + the full-file parse of the 19 pre-2005 cohort zips → `linked_{year}_denomplus.parquet` + the pre-2005 NCHS cohort validation-target sourcing from the 19 guide PDFs; then the HALT 7 §7 end-to-end VERIFY-gate + the soft-flag (ll) per-year 1984-1988 non-anchor re-verification; **halt-and-ask if HALT 7 fails**; then RECEIPT + DECISION_LOG + STATUS; intermediate DO step → no tag; ZERO canonical parquet/schema-CSV/field_specs mutation). DO step 6b (the canonical v3→v4 re-harmonize + schema edit + version bump + VERIFY) = the next session's per-sub-step PRE-FLIGHT + DO.
