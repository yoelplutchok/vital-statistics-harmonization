...
- The harmonization extends forward as NCHS releases new public-use files.
...
The fetal-death file spans 1982–2024 across the 1978-, 1989-, and 2003-revision eras (including the 2003–2004 dual-certificate transition years and 1982–1991); Hispanic origin is unavailable for 1982–1988. The 1989–1991 fetal-death layout was verified by 98,500 raw-byte comparisons with zero mismatches. Natality spans 1968–2024 with resident-birth counts and pre-1990 low-birth-weight and preterm rates validated against published sources. Linked birth–infant death spans 1983–2023 (cohort-linked 1983–2004, period-linked 2016–2023). Matched multiples cover three NCHS publication windows (1995–1997, 1995–2000, 2016–2020) with 143 documentation-table validation targets for Table 1 and Table 2a.
...
Each product ships one stable harmonized schema (natality 84 columns; linked 100 including 1983–2004 cohort fields and an ICD-10 cause bridge; fetal death 89; matched multiples 24). Each `harmonized_schema.csv` lists dtype, allowed values, coverage, comparability class, and source field per era; the full per-era mapping ships as `variable_crosswalk_working.csv` (Supplementary Table S1).
...
**Case study 1: joint-use fetal mortality rates.** Shared column names (`data_year`, `maternal_age`, `maternal_race_bridged`, `hispanic_origin`, `residence_status`) let natality supply stratified live-birth denominators for fetal-death numerators. Pre-computed tabulations and the `joint_use_demo` notebook reproduce *NVSR 73-09* Table 4 byte-exact; convenience denominators ship with the fetal-death product.

**Case study 2: matched multiples mortality.** The `matched_multiples_demo` notebook reproduces **143/143** documentation-table validation targets[^mm_validation] and the complete-twin-set infant mortality rate (10.14 per 1,000), illustrating analyses within a single delivery set.

**Case study 3: cross-revision race-stratified trends.** Stable schemas across the 1978, 1989, and 2003 revisions support continuous race-stratified series; `_x_maternal_race` tabulations and the `cross_race_fetal_mortality` notebook span 1982–2024 fetal mortality across four source eras (2018 bridged-race discontinuity explicit), including Hispanic origin where Ananth and colleagues excluded it for lack of a harmonized column[^ananth2022].
...
[^zenodo_validation]: Natality targets comprise 56 resident-birth counts (1968–2024), 44 pre-1990 low-birth-weight and preterm rate cells (1968–1989), and 149 *Births: Final Data* rate/indicator cells (1990–2024). The Zenodo deposit will receive a docs-only metadata refresh so validation tables match these counts before journal submission.

[^mm_validation]: 41 Table 1-equivalent cells across three publication windows plus 102 Table 2a twin-set cells (34 per window; gender × maternal age × outcome for complete twin sets).
