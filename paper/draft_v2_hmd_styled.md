...
The natality, fetal-death, and linked files are designed for joint use: fetal mortality rates need live-birth denominators stratified like the fetal-death numerator, and perinatal rates additionally need early neonatal death counts from the linked file. Because these products share aligned column names for shared concepts (`maternal_age`, `maternal_race_bridged`, `hispanic_origin`, `data_year`, `residence_status`), joins on `data_year` and demographic strata produce numerator and denominator with consistent semantics. A `live_births_by_year.csv` reference (1995–2002 and 2005–2024) and a stratified-denominator companion ship with the fetal-death product for users who want denominators without loading the full natality file.
...
Earlier roadmaps listed the fetal-death 2003–2004 transition years and the 1982–1991 backward extension as planned; both have now shipped (see above). The roadmap now commits the project to:
