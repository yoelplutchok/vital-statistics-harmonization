...
Build `notebooks/state_reporting_quirks.ipynb` deterministically from this source.

Fifth of five Tier-2 worked-example notebooks per C8.15 §15.C
(`education_gradient.ipynb` + `state_reporting_quirks.ipynb`). This builder
ships C.6.e (notebook 5); receipt + tag are `C8.15-complete` after both
C.6.d and C.6.e ship in the same task.

Run from the repo root:

    python notebooks/_build_state_reporting_quirks.py

The notebook reproduces the four state-level reporting quirks documented
in `fetal_death/COMPARABILITY.md` (Louisiana plurality 1992-1994; Oklahoma
Hispanic 1992-2002; Maryland Hispanic 1992-1998; Massachusetts Hispanic
1992-1997) plus the V1-era plurality '5' miscoding 2005-2013, and closes
...
The reason this notebook reads raw is operational: state codes
(`STATEFET`/`STATERES` V2 era; `OSTATE`/`MRSTATEPSTL` V1 era) are dropped
during harmonization, so per-state reproduction of the COMPARABILITY-
cited counts requires the raw layout. V1 era state codes are
operationally suppressed by NCHS in the public-use file (100% blank
across all 2005+ years; mirrors the C8.9 natality state-suppression
finding); the 2005-2013 plurality '5' miscoding is therefore documented
aggregate-only without per-state attribution.

Cell layout:

  Section 0 — Per-year-raw-parquet helper + canonical fetal-death filter
              recipe (`RESTATUS != '4'` for V2 era; `TABFLG == '2' AND
              RESTATUS != '4'` for V1 era).
  Section 1 — Louisiana plurality non-reporting 1992-1994 (V2 era;
              STATEFET=19; reproduces COMPARABILITY's "1,686 of 1,714
              LA-occurrence records" with DPLURAL=9 byte-exact).
  Section 2 — Oklahoma Hispanic non-reporting 1992-2002 (V2 era;
              STATEFET=37; ORMOTH=9 across all 11 years).
  Section 3 — Maryland 1992-1998 + Massachusetts 1992-1997 Hispanic
              non-reporting (V2 era; STATEFET=21, 22; ORMOTH=9 with
              transition-year inflection).
  Section 4 — V1 era plurality '5' miscoding 2005-2013 (aggregate-only
              per V1 state suppression); recommended researcher recipe
              from COMPARABILITY line 171.
  Section 5 — V1 era state suppression note (mirrors C8.9 finding).
  Section 6 — Pass/fail summary + soft-flag (f) closure narrative.

The notebook validates against the explicit counts cited in
`fetal_death/COMPARABILITY.md` (lines 162-172, 267-269, 273-275). Each
section asserts the reproduced count matches the documented count
byte-exact.
"""
...
OUTPUT = REPO_ROOT / "notebooks" / "state_reporting_quirks.ipynb"
