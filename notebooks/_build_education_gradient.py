...
def build() -> nbformat.NotebookNode:
    ...
    nb.cells = [
        ...
        code(
            ...
            "]\n"
            "\n"
            "def load_nat_range(year_lo: int, year_hi: int) -> pd.DataFrame:\n"
            "    \"\"\"Resident-only slice for a year window (avoids loading 201M-row full file).\"\"\"\n"
            "    df = pq.read_table(\n"
            "        NAT,\n"
            "        columns=NAT_COLS,\n"
            "        filters=[('data_year', '>=', year_lo), ('data_year', '<=', year_hi)],\n"
            "    ).to_pandas()\n"
            "    return df[df['residence_status'] != 4].copy()\n"
            "\n"
            "pf_nat = pq.ParquetFile(NAT)\n"
            "year_min, year_max = 9999, 0\n"
            "for batch in pf_nat.iter_batches(columns=['data_year'], batch_size=5_000_000):\n"
            "    col = batch.column('data_year').to_numpy()\n"
            "    year_min = min(year_min, int(col.min()))\n"
            "    year_max = max(year_max, int(col.max()))\n"
            "print(f'Natality v2 raw: {pf_nat.metadata.num_rows:,} rows; years {year_min}-{year_max}')\n"
            "\n"
            "linked = pq.read_table(\n"
            "    LINKED,\n"
            "    columns=['data_year', 'residence_status', 'certificate_revision', 'maternal_education_cat4', 'infant_death'],\n"
            "    filters=[('data_year', '==', 2022)],\n"
            ").to_pandas()\n"
            "linked['infant_death'] = pd.to_numeric(linked['infant_death'], errors='coerce').fillna(0).astype('int8')\n"
            "linked_us = linked[linked['residence_status'] != 4].copy()\n"
            "print(f'Linked v3 (2022 cohort slice): {len(linked_us):,} resident rows')"
        ),
        md(
        ...
        code(
            "# Per-year null rate (aggregate across both revisions) — one year per read\n"
            "by_year_rows = []\n"
            "for y in range(1990, 2025):\n"
            "    sub = load_nat_range(y, y)\n"
            "    by_year_rows.append({\n"
            "        'data_year': y,\n"
            "        'n': len(sub),\n"
            "        'null_rate': sub['maternal_education_cat4'].isna().mean(),\n"
            "    })\n"
        ...
        code(
            ...
            "    sub = load_nat_range(y, y).assign(meduc_null=lambda d: d['maternal_education_cat4'].isna())\n"
            "    rev_parts.append(\n"
            "        sub.groupby(['data_year', 'certificate_revision'], observed=True)\n"
            "        .agg(n=('meduc_null', 'size'), null_rate=('meduc_null', 'mean'))\n"
            "        .reset_index()\n"
            "    )\n"
            "rev_year = pd.concat(rev_parts, ignore_index=True)\n"
        ...
        code(
            ...
            "    return out\n"
            "\n"
            "nat_era1 = load_nat_range(1990, 2002)\n"
            "era1 = preterm_by_edu(nat_era1, 1990, 2002, '1990-2002')\n"
        ...
        code(
            "nat_era3 = load_nat_range(2014, 2024)\n"
            "era3 = preterm_by_edu(nat_era3, 2014, 2024, '2014-2024')\n"
        ...
        code(
            "year_2022 = linked_us.dropna(subset=['maternal_education_cat4', 'infant_death']).copy()\n"
            "year_2022['infant_death_int'] = year_2022['infant_death'].astype('int8')\n"
            "imr_2022 = (\n"
            "    year_2022\n"
            "    .groupby('maternal_education_cat4')\n"
        ...
        code(
            ...
            "rev_only = load_nat_range(2009, 2013)\n"
            "rev_only = rev_only[rev_only['certificate_revision'] == 'revised_2003'].copy()\n"
