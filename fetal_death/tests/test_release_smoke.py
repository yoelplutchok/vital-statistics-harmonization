"""V2.0 release smoke suite.

Eight tests that must pass before any V2.x release. Each guards an invariant
that an audit or a previous regression specifically called out:

  1. Both parquet artifacts load and have the V2.0 advertised row count.
  ...
  3. Year coverage is exactly the 29 years documented in README.md
     (1992-2002 + 2005-2022; 2003/2004 deferred to V2.1).
  4. data_year == int(delivery_year) holds row-by-row
     (the B-PUB-3 promise that data_year is a faithful int sibling).
  5. Every V2 era row (1992-2002) carries version_flag='S'
     (synthesized in harmonize.py because the source files have no native
     VERSION field; regressed once during B4 development).
  ...
  7. Per-year row counts are within +/-5% of the published V2.0 reference.
...
EXPECTED_ROW_COUNT = 1_634_195
EXPECTED_HARMONIZED_COLS = 73
EXPECTED_DERIVED_COLS = 89
EXPECTED_YEARS = tuple(list(range(1992, 2003)) + list(range(2005, 2023)))  # 29 years

# Per-year row counts at V2.0 release. Drift +/-5% triggers test 7.
EXPECTED_YEAR_ROWS = {
    1992: 70929, 1993: 71181, 1994: 66091, 1995: 63170, 1996: 65163,
    1997: 64002, 1998: 63438, 1999: 63875, 2000: 63132, 2001: 54442,
    2002: 55281, 2005: 53333, 2006: 52714, 2007: 60973, 2008: 60154,
    2009: 56685, 2010: 58079, 2011: 58361, 2012: 56201, 2013: 54028,
    2014: 52872, 2015: 51490, 2016: 51391, 2017: 49170, 2018: 47676,
    2019: 46007, 2020: 41816, 2021: 42428, 2022: 40113,
}

# 2010 NVSR-comparable anchor: tabulation_flag=='2' & residence_status!='4'.
# Verified during AUDIT_PUBLICATION_2 (2026-05-02).
...
def test_year_coverage_is_29_years(harmonized_year_invariants: pd.DataFrame) -> None:
    ...
    # 2003 and 2004 explicitly deferred to V2.1 (per README.md version roadmap)
    assert 2003 not in years
...
def test_v2_era_version_flag_is_S(harmonized_year_invariants: pd.DataFrame) -> None:
    df = harmonized_year_invariants
    v2 = df[df["data_year"].between(1992, 2002)]
    assert len(v2) > 0, "V2 era slice unexpectedly empty"
    non_s = (v2["version_flag"] != "S").sum()
    assert non_s == 0, (
        f"{non_s} V2-era rows (1992-2002) have version_flag != 'S'. "
        "harmonize.py is supposed to synthesize 'S' for the entire 1989-revision era."
    )
