"""DESIGN: tracks-current-state — asserts each of the 346 canonical parquet
...
Convention 1 SHAPE-not-VALUE for the structural anchors (346 total
columns; 73 + 89 + 84 + 100 per-parquet split; the linked 97->100 is the
...
EXPECTED_COLUMN_COUNTS = {
    ...
    "natality_v3_linked_harmonized_derived": 100,
}
EXPECTED_TOTAL = sum(EXPECTED_COLUMN_COUNTS.values())  # 346 (73+89+84+100)
...
def test_baseline_anchor_row_count() -> None:
    """346 = 73 + 89 + 84 + 100 columns total in the committed baseline
    (C8.18 6b: 94->97; LINK-ICD10: 97->100)."""
    rows = load_baseline_csv(latest_baseline_path())
    assert len(rows) == EXPECTED_TOTAL, (
        f"expected {EXPECTED_TOTAL} baseline rows (73+89+84+100); got {len(rows)}"
    )
